IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'sp_yf_dj_djmxcx' 
	   AND 	  type = 'P')
    DROP PROCEDURE sp_yf_dj_djmxcx
GO
CREATE PROCEDURE sp_yf_dj_djmxcx
(
 	@ywlx char(3),
	@dtpdjsj1 varchar(30),
	@dtpdjsj2 varchar(30),
...
begin
	 ...
	 YPpH 批号,
	 b.lsj 零售价,b.ypdw 单位, '+
	 'zcs 帐存数,zclsje 帐存金额,pcs 盘存数,pclsje 盘存金额,(zcs-pcs) 盈亏数,
	 (pclsje-zclsje) 盈亏金额,'+
...
if @ywlx='005' 
begin
	 set @ssql='select 0 序号,dbo.fun_getdeptname(a.deptid) 仓库名称,c.s_yppm 品名,
	 c.s_ypspm 商品名,s_ypgg 规格,s_sccj 厂家,tjsl 数量,b.ypdw 单位, '+
	 'b.yppch 批次号,kcid ,  ypfj 原批发价,xpfj 现批发价,ylsj 原零售价,xlsj 现零售价,(xlsj-ylsj) 单位差价,tlsje 调价金额, '+
...
--当前业务表。。除调价和盘点业务
	 set @ssql='insert into #djmx(id,djh,deptid,ywlx,wldw,jsr,rq,djrq,shrq,fph,fprq,bz,djid,cjid,ypph,ypxq,ypkl,ypsl,ypdw,jhj,pfj,lsj,jhje,pfje,lsje,shdh,sdjh,fkbl,yppch,kcid)'+
	 'select a.id,a.djh,a.deptid,a.ywlx,wldw,jsr,rq,djrq,shrq,fph,fprq,a.bz,djid,cjid,ypph,ypxq,ypkl,ypsl,ypdw,jhj,pfj,lsj,jhje,pfje,lsje,b.shdh,sdjh,b.fkbl,yppch,kcid '+ 
	 ' from vi_yf_dj a,vi_yf_djmx b where a.id=b.djid and a.deptid in(select deptid from #deptid) ';
	 ...
	 if @ywlx='001' or @ywlx='002' or @ywlx='019' 
	 	 set @ssfun='dbo.fun_yp_ghdw(wldw) 往来单位';
	 else
	    set @ssfun='dbo.fun_getdeptname(wldw) 往来单位';
 
	 set @ssql='select 0 序号,dbo.fun_getdeptname(deptid) 仓库名称,rtrim(dbo.fun_yf_ywlx(ywlx)) 业务类型,s_yppm 品名,s_ypspm 商品名,s_ypgg 规格,'+
	 's_sccj 厂家,
	  YPPCH 批次号,kcid,
	  YPph 批号,rtrim(ypxq) 效期,a.ypkl 扣率,a.jhj 进价,a.lsj 零售价,cast(ypsl as float) 数量,a.ypdw 单位,jhje 进货金额,lsje 零售金额,'+
	 ' (lsje-jhje) 进零差额,djh 单据号,sdjh 月单据号,cast(rq as datetime)  单据日期,shdh 送货单号,'+@ssfun+',DBO.fun_yp_ywy(jsr) 业务员,fph 发票号,'+
	 'fprq 发票日期,dbo.Fun_GetDate(djrq) 登记日期,shrq  审核日期,txm 条形码,a.pfj 批发价,pfje 批发金额,cast(a.fkbl*100 as decimal(15,2)) 付款比例, cast(a.jhje*a.fkbl as decimal(15,3)) 付款金额 from #djmx a,yp_ypcjd b where a.cjid=b.cjid order by id';
	exec(@ssql)
