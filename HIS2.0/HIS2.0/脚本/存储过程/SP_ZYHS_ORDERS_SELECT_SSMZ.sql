IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_ZYHS_ORDERS_SELECT_SSMZ]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_ZYHS_ORDERS_SELECT_SSMZ]
GO
create  PROCEDURE [dbo].[SP_ZYHS_ORDERS_SELECT_SSMZ]
 (
...
BEGIN
	...
	left join JC_HOITEMDICTION ho on A.HOITEM_ID=ho.ORDER_ID and A.XMLY=2 --Add By Tany 2015-06-18
	WHERE (A.MNGTYPE=@SELTYPE OR A.MNGTYPE=@SELTYPE2)
              AND A.DEPT_ID IN (SELECT DEPTID FROM SS_DEPT where deptid in (select dept_id from jc_wardrdept where ward_id=@wardid or @wardid='-1'))
