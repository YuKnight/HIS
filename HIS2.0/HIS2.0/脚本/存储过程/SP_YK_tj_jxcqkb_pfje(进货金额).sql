...
alter PROCEDURE SP_YK_tj_jxcqkb_pfje  
 (@year int,   
  @month INTEGER,  
  @deptid integer,  
  @yplx integer,  
...
begin  
 if @yplx=0   
     insert into #TEMP(xmmc,je,ywlx,sortid,jdfx,jhje) select '上期结存',sum(jclsje),'000',0,0,sum(round(JCJHJE,2)) from YK_YMJC where  nf=@sqyear and yf=@sqmonth  and deptid in(select deptid from #deptid)  
 else  
     insert into #TEMP(xmmc,je,ywlx,sortid,jdfx,jhje) select '上期结存',sum(jclsje),'000',0,0,sum(round(JCJHJE,2)) from YK_YMJC a,vi_yp_ypcd b where a.cjid=b.cjid and b.yplx=@yplx and nf=@sqyear and yf=@sqmonth and deptid in(select deptid from #deptid)  
end  
  
insert into #TEMP(wldw,xmmc,je,jhje,ywlx) select 0, ywmc,0,0,ywlx from yk_ywlx where ywzt=1 and ywlx not in('008','012','005');  
...
begin  
    if @yplx=0   
     insert into #TEMP(xmmc,je,jhje,ywlx,sortid,jdfx) select '本期结存',sum(jclsje),sum(round(JCJHJE,2)),'999',1000,1 from YK_YMJC   
  where nf=@year and yf=@month  and deptid in(select deptid from #deptid)  
    else  
     insert into #TEMP(xmmc,je,jhje,ywlx,sortid,jdfx) select '本期结存',sum(jclsje),sum(round(JCJHJE,2)),'999',1000,1 from YK_YMJC a,vi_yp_ypcd b   
...
--update #TEMP set xmmc='药品报溢',je=je,sortid=6,jdfx=0 where ywlx='007';  
...
--  
--IF EXISTS (select * from #temp where ywlx='012' and je>=0 and jhje>=0)  
-- update #TEMP set xmmc='账务调整增加',je=je,sortid=8,jdfx=0 where ywlx='012' and je>=0;  
--IF EXISTS (select * from #temp where ywlx='012' and je>=0 and jhje<0 and jdfx is null)  
--begin  
-- insert into #TEMP(wldw,xmmc,je,ywlx,jhje,sortid,jdfx)select 0,'账务调整减少',0,'012',abs(jhje),27,1 from #temp where ywlx='012' and je>=0 and jdfx is null;  
-- update #TEMP set xmmc='账务调整增加',je=je,jhje=0,sortid=8,jdfx=0 where ywlx='012' and je>=0 and jdfx is null;  
--end  
...
insert into #TEMP(xmmc,je,jhje,ywlx,sortid,jdfx) select '账务调整增加',0,0,'012',8,0  
insert into #TEMP(xmmc,je,jhje,ywlx,sortid,jdfx) select '调价升溢',0,0,'005',7,0  
...
--update #TEMP set xmmc='调价损失',je=abs(je),jhje=abs(jhje),sortid=26,jdfx=1 where ywlx='005' and je<0;  
--IF EXISTS (select * from #temp where ywlx='012' and je<=0 and jhje<=0 and jdfx is null)  
--begin  
-- update #TEMP set xmmc='账务调整减少',je=abs(je),jhje=abs(jhje),sortid=27,jdfx=1 where ywlx='012' and je<=0 and jdfx is null;  
--end  
--IF EXISTS (select * from #temp where ywlx='012' and je<=0 and jhje>0 and jdfx is null)  
--begin  
-- insert into #TEMP(wldw,xmmc,je,ywlx,jhje,sortid,jdfx)select 0,'账务调整增加',0,'012',jhje,27,0 from #temp where ywlx='012' and je<=0 and jdfx is null;  
-- update #TEMP set xmmc='账务调整减少',je=abs(je),jhje=0,sortid=27,jdfx=1 where ywlx='012' and je<=0 and jdfx is null;  
--end  
...
insert into #TEMP(xmmc,je,jhje,ywlx,sortid,jdfx) select '账务调整减少',0,0,'012',27,1  
insert into #TEMP(xmmc,je,jhje,ywlx,sortid,jdfx) select '调价损失',0,0,'005',26,1 
...
update #TEMP set xmmc='批次调整',je=ABS(je),sortid=28,jdfx=1 where ywlx='034' ----ncq 20140612

--插入借方数据   
insert into #TEMP1(xmmc,je,jhje) select xmmc,je,coalesce(jhje,0) from #TEMP where (je<>0 and jdfx=0)  or ywlx='000' or  (je=0 and jhje<>0 and jdfx=0) order by sortid  
...
set @id=1;  
declare t2 cursor for select xmmc,je,coalesce(jhje,0) from #TEMP where (je<>0 and jdfx=1) or ywlx='999' or (je=0 and jhje<>0 and jdfx=1) order by sortid  
open t2  
