...
if not EXISTS(select * from #DJMX where kcid not in(select kcid from YF_KCPH where DEPTID=@deptid))
		select 
		a.cjid,shh 货号,s_yppm 品名,s_ypspm 商品名,s_ypgg 规格,s_sccj 厂家,ypdw 单位,  
		...
		where abs(jcsl+bqsl-kcsl)>0.001 or ABS(jclsje+bqlsje-kclsje)>=0.01 --Modify By Tany 2015-03-31 把数量也过滤一下
