IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'SP_YF_tj_jxcqkb' 
	   AND 	  type = 'P')
    DROP PROCEDURE SP_YF_tj_jxcqkb
GO


CREATE PROCEDURE SP_YF_tj_jxcqkb
 ( @year int, 
   @month INTEGER,
   @deptid integer,
   @yplx integer
...
begin
	if @yplx=0 
	   	insert into #temp(xmmc,je,ywlx,sortid,jdfx,jhje) select '上期结存',round(sum(jclsje),2),'000',0,0,sum(jcjhje) from YF_YMJC where  nf=@sqyear and yf=@sqmonth and deptid=@deptid;
	else
	    insert into #temp(xmmc,je,ywlx,sortid,jdfx,jhje) select '上期结存',round(sum(jclsje),2),'000',0,0,sum(jcjhje) from YF_YMJC a,vi_yp_ypcd b where a.cjid=b.cjid and b.yplx=@yplx and nf=@sqyear and yf=@sqmonth and deptid=@deptid;
end


insert into #temp(wldw,xmmc,je,jhje,ywlx) select 0, ywmc,0,0,ywlx from yF_ywlx where ywzt=1;
...
begin
    if @yplx=0 
	   	insert into #TEMP(xmmc,je,jhje,ywlx,sortid,jdfx) select '本期结存',sum(jclsje),sum(jcjhje),'999',1000,1 from Yf_YMJC 
		where nf=@year and yf=@month and deptid=@deptid;
    else
	   	insert into #TEMP(xmmc,je,jhje,ywlx,sortid,jdfx) select '本期结存',sum(jclsje),sum(jcjhje),'999',1000,1 from Yf_YMJC a,vi_yp_ypcd b 
...
update #temp set xmmc='药品退货',je=je*(-1),sortid=3,jdfx=0 where ywlx='002';
...
update #temp set xmmc=coalesce(dbo.fun_yp_ghdw(wldw),'')+'入库',je=je,sortid=6,jdfx=0 where ywlx='019';
...
insert into #temp(xmmc,je,jhje,ywlx,sortid,jdfx) select '药品盘盈',sum(lsje) je,sum(jhje),'008',10,0 from #djmx where ywlx='008' and lsje>0;
...
update #temp set xmmc='调价损失',je=abs(je),sortid=29,jdfx=1 where ywlx='005' and je<0;
update #temp set xmmc='账务调整减少',je=abs(je),sortid=30,jdfx=1 where ywlx='012' and je<0;
insert into #temp(xmmc,je,jhje,ywlx,sortid,jdfx) select '药品盘亏',round(abs(sum(lsje)),2) je,abs(sum(jhje)),'008',31,1 from #djmx where ywlx='008' and lsje<0;



insert into #TEMP1(xmmc,je,jhje) select xmmc,je,jhje from #TEMP where (je<>0 and jdfx=0) or ywlx='000' order by sortid
...
set @id=1;
declare t2 cursor local for select xmmc,je,coalesce(jhje,0) from #TEMP where (je<>0 and jdfx=1) or ywlx='999' or (je=0 and jhje<>0) order by sortid
open t2
