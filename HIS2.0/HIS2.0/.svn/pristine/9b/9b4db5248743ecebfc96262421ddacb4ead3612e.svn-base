IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_yp_tj_jxchzb_yplx_hh]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_yp_tj_jxchzb_yplx_hh]
GO
CREATE PROCEDURE [dbo].[sp_yp_tj_jxchzb_yplx_hh]     
(    
 @yk int,    
 @jhjetj int,    
 @year int,    
...
exec sp_yp_tj_jxchzb_yplx_hh @yk=0,@jhjetj=0,@year=2014,@month=7,@err_code=@err_code output,@err_text=@err_text output    
...
if @yk=1     
 insert into #yjks    
 select  rtrim(KSMC)+rtrim(dbo.fun_yp_yplx(c.yplx)) as xmmc ,cast(c.YPLX as varchar(50))+'000'+CAST(left('00000',5-LEN(a.deptid)) as varchar(50))+cast(a.deptid as varchar(30)),a.pxxh from yp_yjks a inner join jc_dept_property b on a.DEPTID=b.DEPT_ID and (b.NAME like '%后湖%' or b.NAME like '%静脉%') and kslx='药库'
 inner join YP_GLLX c on a.DEPTID=c.DEPTID  group by  KSMC,c.YPLX,a.pxxh,a.deptid order by a.pxxh  
else    
 insert into #yjks    
 select  rtrim(KSMC)+rtrim(dbo.fun_yp_yplx(c.yplx)) as xmmc ,cast(c.YPLX as varchar(50))+'000'+CAST(left('00000',5-LEN(a.deptid)) as varchar(50))+cast(a.deptid as varchar(30)),a.pxxh from yp_yjks a inner join jc_dept_property b on a.DEPTID=b.DEPT_ID and (b.NAME like '%后湖%' or b.NAME like '%静脉%') and kslx='药房'
 inner join YP_GLLX c on a.DEPTID=c.DEPTID  group by  KSMC,c.YPLX,a.pxxh,a.deptid order by a.pxxh    
