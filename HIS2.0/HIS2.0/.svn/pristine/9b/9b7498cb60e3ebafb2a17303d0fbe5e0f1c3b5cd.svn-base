...
namespace ts_mzys_class
{
    ...
    public class mzys_jzjl
    {
        ...
        /// </summary>
        /// <param name="ghxxid">挂号信息ID</param>
        /// <param name="reciveDept">接收科室</param>
        /// <param name="applyDept">申请发起科室</param>
        /// <param name="applyDoc">申请发起人</param>
        /// <param name="message">错误消息</param>
        /// <param name="database">数据访问类</param>
        /// <returns></returns>
        public static bool AllowReferral( Guid ghxxid , int reciveDept , int applyDept , int applyDoc , out string message , RelationalDatabase database )
        {
            #region 防止通过先转诊到科室，在接诊的方法，抢别科的病人 add by wangzhi 2014-10-10
            DataRow rowGHXX = database.GetDataRow( string.Format( "select * from mz_ghxx where ghxxid='{0}'" , ghxxid ) );
            int ghks = Convert.ToInt32( Convertor.IsNull( rowGHXX["ghks"] , "0" ) ); //挂号科室
            int ghys = Convert.ToInt32( Convertor.IsNull( rowGHXX["ghys"] , "0" ) ); //挂号医生
            int jzks = Convert.ToInt32( Convertor.IsNull( rowGHXX["jzks"] , "0" ) ); //接诊科室 
            int jzys = Convert.ToInt32( Convertor.IsNull( rowGHXX["jzys"] , "0" ) ); //接诊医生 

            int jsks = reciveDept;//接收科室
            int brks = jzks == 0 ? ghks : jzks;
            message = "";
            if ( brks != applyDept )
            {
                message = "转诊申请需要由当前病人的挂号科室或接诊科室发起";
                return false;
            }
            else
            {
                int doc = jzys == 0 ? ghys : jzys;
                if ( doc != 0 && doc != applyDoc )
                {
                    message = "转诊申请需要由病人的挂号医生或接诊医生发起";
                    return false;
                }
            }
            return true;
