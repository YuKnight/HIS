...
namespace ts_mzys_class 
{
    public class mzys_yjsq
    {
        ...
        public static int UpdateSfbz_QX(string yjsqid, string sdate, int type ,RelationalDatabase _DataBase)
        {
            string ssql = " update YJ_MZSQ set bqxsfbz=1,qxsfrq='" + sdate + "' where yjsqid = '" + yjsqid + "' and bsfbz=1 and bscbz=0 ";
            return _DataBase.DoCommand(ssql);
        }
        ...
        /// 获取医技申请
        /// </summary>
        /// <param name="yzid">医嘱ID，即 MZ_HJB.HJID</param>
        /// <param name="hjmxid">划价明细ID，如果是套餐，则可能为Empty</param>
        /// <param name="yzxmid">医嘱项目ID</param>
        /// <param name="tcid">套餐ID，不是套餐则为0</param>
        /// <returns></returns>
        public static DataRow GetYjsqInfo( Guid yzid , Guid hjmxid , long yzxmid, int tcid ,RelationalDatabase database)
        {
            string sql = string.Format( "select * from yj_mzsq where yzid='{0}'" , yzid );
            if ( tcid == 0 )
                sql = sql + string.Format( " and hjmxid='{0}'" , hjmxid );
            else
                sql = sql + string.Format( " and yzxmid={0}" , yzxmid );
