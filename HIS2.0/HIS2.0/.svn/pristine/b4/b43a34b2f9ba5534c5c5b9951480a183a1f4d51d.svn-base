IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'sp_yp_tj_rkhztj' 
	   AND 	  type = 'P')
    DROP PROCEDURE sp_yp_tj_rkhztj
GO
CREATE PROCEDURE sp_yp_tj_rkhztj 
(
	@deptid int,
	@yk int,
	@jhjetj int,
...
exec sp_yp_tj_rkhztj @deptid=94,@yk=1,@jhjetj=0,@year=2014,@month=7,@err_code=@err_code output,@err_text=@err_text output
select @err_code,@err_text

declare @err_code int
declare @err_text varchar(300)
exec sp_yp_tj_rkhztj @deptid=142,@yk=0,@jhjetj=0,@year=2014,@month=7,@err_code=@err_code output,@err_text=@err_text output
select @err_code,@err_text
...
begin  
if @jhjetj=1 
        select dbo.fun_yp_wldwmc(c.wldwlx,wldw) 往来单位,sum(case when a.ywlx not in('002') then JHJE else 0 end) 入库金额,
        sum(case when a.YWLX in('002') then jhje else 0 end) 退货金额,
		sum(dbo.fun_yk_drt(a.ywlx,JHJE)) as 实际金额 
        ...
        select dbo.fun_yp_wldwmc(c.wldwlx,wldw) 往来单位,sum(case when a.ywlx not in('002') then LSJE else 0 end) 入库金额,
        sum(case when a.YWLX in('002') then LSJE else 0 end) 退货金额,
		sum(dbo.fun_yk_drt(a.ywlx,LSJE)) as 实际金额 
		from  YP_TJ_YMJCMX a  inner join #tempyjjl b on a.yjid=b.yjid inner join #ywlx c on a.YWLX=c.ywlx  
        inner join yp_ypcjd D on a.CJID=d.CJID where tjlx='本期增加' 
        group by a.wldw,c.WLDWLX order by wldw 
end
...
begin  
if @jhjetj=1 
        select dbo.fun_yp_wldwmc(c.wldwlx,wldw) 往来单位,sum(case when a.ywlx not in('002','004') then JHJE else 0 end) 入库金额,
        sum(case when a.YWLX in('002','004') then jhje else 0 end) 退货金额,
		sum(dbo.fun_yf_drt(a.ywlx,JHJE)) as 实际金额 
		from  YP_TJ_YMJCMX a  inner join #tempyjjl b on a.yjid=b.yjid inner join #ywlx c on a.YWLX=c.ywlx  
        inner join yp_ypcjd D on a.CJID=d.CJID where tjlx='本期增加' 
        group by a.wldw,c.WLDWLX order by wldw 
else
        select dbo.fun_yp_wldwmc(c.wldwlx,wldw) 往来单位,sum(case when a.ywlx not in('002','004') then LSJE else 0 end) 入库金额,
        sum(case when a.YWLX in('002','004') then LSJE else 0 end) 退货金额,
		sum(dbo.fun_yf_drt(a.ywlx,LSJE)) as 实际金额 
		from  YP_TJ_YMJCMX a  inner join #tempyjjl b on a.yjid=b.yjid inner join #ywlx c on a.YWLX=c.ywlx  
        inner join yp_ypcjd D on a.CJID=d.CJID where tjlx='本期增加' 
        group by a.wldw,c.WLDWLX order by wldw 
end
