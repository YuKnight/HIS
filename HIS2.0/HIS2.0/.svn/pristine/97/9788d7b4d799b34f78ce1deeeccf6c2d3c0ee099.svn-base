...
namespace ts_mz_class
{
    ...
    public class Mz_YYgh
    {
        ...
        public static void YYGH_save(Guid yyid, Mz_YYgh.YYgh_Sort yylx, string kh, string brxm, string xb, object csrq,
            ...
            out string newPdxh, out Guid newYYid, out int err_code, out string err_text, RelationalDatabase _DataBase,string Bz)
        {
            ...
            try
            {
                ...
                parameters[31].Text = "@bz";
                parameters[31].Value = Bz;
        ...
        /// <summary>
        /// 获取预约挂号列表 Modify By zp 2014-09-26 新增科室id、医生id 入参
        /// Modify by ZP 2014-10-06 新增预约类型
        /// </summary>
        /// <param name="sfzh">身份证号</param>
        /// <param name="kh">卡号</param>
        /// <param name="brxm">病人姓名</param>
        ...
        public static DataTable GetYYghInfo(string yyid, string sfzh, string kh, string yzm,
           YYgh_Status status ,string bdate,string edate,int klx,int deptId,int docId,YYgh_Sort yylx,
            out int err_code,out string err_text, RelationalDatabase _DataBase)
        {
                ...
                try
                {
                    ...
                    parameters[9].Text = "@DocId";
                    parameters[9].Value = docId;

                    parameters[10].Text = "@Yylx";
                    parameters[10].Value = (int)yylx;
        ...
        [Obsolete("",true )]
        public static DataTable GetYYghInfo(string yyid, string sfzh, string kh, string yzm,
           YYgh_Status status, string bdate, string edate, int klx,
            out int err_code, out string err_text, RelationalDatabase _DataBase)
        {
        ...
        /// <summary>
        /// 获取医生排班  add by zp 2013-4-11 Modify by zp 2014-08-08 ZZJB获取级别表的医生级别
        /// </summary>
            ...
            int dept_id,int emp_id, RelationalDatabase _DataBase)
        {
            ...
            try
            {
                string ssql = @" select ID,PBKSID,PBKS,
                case when PBLX=1 then '上午' when PBLX=2 then '下午' else '晚上' end as 排班类型,
                pblx,PBSJ,YSID,YSXM,B.TYPE_NAME AS ZZJB,ZZJBID,JJRBZ,JZ,ZJID_QC
                from JC_MZ_YSPB A 
                INNER JOIN JC_DOCTOR_TYPE B ON A.ZZJBID=B.TYPE_ID
                where BSCBZ=0 AND PBSJ>='" + bdate + "' and PBSJ<='" + edate + @"'";
                if (dept_id > 0)
                {
            ...
            long jgbm, RelationalDatabase _DataBase, out int err_code, out string err_text, out DataTable dt_ghxx)
        {
            ...
            try
            {
                ...
                sql += " order by b.zfbz ";//没有作废的排前面，不然会出现作废的卡号后，又重新发给另外一个人后，这里就会出现问题
                ...
                sql = @" select a.GHXXID as 挂号信息id,dbo.fun_getDeptname(a.GHKS) as 科室名称,a.GHDJSJ as 挂号登记时间,ISNULL( b.TYPE_NAME,'') as 医生级别名称,
                  ...
                  a.PDXH as 候诊号,dbo.fun_getEmpName(a.GHYS) as 挂号医生名称,GHJE as 挂号金额,'' as 诊室位置,a.GHSJ 挂号日期,a.BRXXID as 病人信息id
        ...
        public static bool UpdateMoneyFlag(Guid YYID, string brxm, string ghrq, RelationalDatabase _DataBase)
        {
           ...
           AND CONVERT(varchar(10),YYRQ,120)=CONVERT(varchar(10),'" + ghrq + "',120)";
                if (_DataBase.DoCommand(sql) > 0)
                    return true;
            }
            catch(Exception ea)
