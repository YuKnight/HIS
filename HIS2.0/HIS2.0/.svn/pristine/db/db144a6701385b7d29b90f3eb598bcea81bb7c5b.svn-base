...
begin
  ...
  select 0 序号,s_yppm 品名,s_ypspm 商品名,s_ypgg 规格,s_sccj 厂家,lsj 零售价,s_ypdw 单位,cast(rksl as float) 入库数,-(cast(cksl as float)) 出库数,
   cast(kcl as float) 库存数 from #temp a inner join yp_ypcjd b on a.cjid=b.cjid 
end
