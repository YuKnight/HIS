...
namespace TrasenClasses.DatabaseAccess
{
    /// <summary>
	/// Odbc 的摘要说明。 
    /// jianqg 2013-4-1 增加 本类 处理odbc 没有详加测试，初始设想是处理dbf数据库，以后可以扩展为其他的odbc
	/// </summary>
	public class Odbc: RelationalDatabase
    {

        /// <summary>
		/// 数据库平台名称
		/// </summary>
		protected const string VENDOR_NAME = "Odbc";
		/// <summary>
		/// 构造一Odbc
		/// </summary>
		public Odbc()
		...
		public override void CreateCommandBuilder(IDataAdapter _adapter)
		{
			...
			OdbcCommandBuilder cb = new OdbcCommandBuilder((OdbcDataAdapter)_adapter);
		}
		#endregion
		...
		public override int DoCommand(string commandText)
		{
			...
			if(isInTransaction) cmd.Transaction= (OdbcTransaction)transaction;
		...
		{
			return new OdbcParameter(parameterName,values);
		}
		/// <summary>
		/// 返回一个参数对象
		/// </summary>
		...
		{
			return new OdbcParameter(parameterName,(OdbcType)dataType,size);
		}
		/// <summary>
		/// 返回一个参数对象
		/// </summary>
		...
		{
			OdbcParameter para= new OdbcParameter(parameterName,(OdbcType)dataType,size);
			para.Value = values;
			return para;
		}
		/// <summary>
		...
		{
			OdbcCommand cmd=new OdbcCommand(commandText);
			cmd.Connection=(OdbcConnection)this.connection;				//添加连接
			DataRow r ;
		...
		{
			return "VALUES IDENTITY_VAL_LOCAL() ";// jianqg 2013-4-1 未处理 
		}
		/// <summary>
		/// 获取服务器时间的SQL语句(默认为IBM DB2数据库语句）
		/// </summary>
		...
		{
            return "VALUES CURRENT TIMESTAMP"; // jianqg 2013-4-1 未处理
		}
		/// <summary>
        ...
        /// <summary>
        ///  初始化数据库（刚被初始化的数据库的连接是打开的） 使用 this.cnnString
        /// </summary>
        public void Initialize()
        {
            Initialize(this.cnnString);
        }
		/// <summary>
		/// 返回参数的字符串形式
		/// </summary>
