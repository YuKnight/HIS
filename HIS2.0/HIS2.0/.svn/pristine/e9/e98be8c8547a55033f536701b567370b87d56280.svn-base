IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_ZYHS_ORDERPRINT]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_ZYHS_ORDERPRINT]
GO
create  PROCEDURE [dbo].[SP_ZYHS_ORDERPRINT]    
 (  
...
DECLARE @CS_CancelMemo  varchar(50) --add by jchl 2014_09_30 取消原因
...
declare @cfg7196 varchar(10)
set @cfg7196=(select  isnull(config,'0')  from jc_config where id=7196)
...
select * into #temporder from  (select ORDER_ID,GROUP_ID,INPATIENT_ID,DEPT_ID,WARD_ID,MNGTYPE,NTYPE,ORDER_DOC,HOITEM_ID,ITEM_CODE,XMLY,
               --case when (@cfg7177=0 or XMLY=2 ) then  ORDER_CONTEXT else substring(ORDER_SPEC,1,CHARINDEX('%',ORDER_SPEC))+ORDER_CONTEXT  
               --end ORDER_CONTEXT,
                 case when (@cfg7177=0 or XMLY=2 )
               --ORDER_USAGE like '%皮试%'--add by 岳成成 2014-07-29
               --(select top 1 YPSPMBZ  from VI_YP_YPCD d where d.cjid=a.HOITEM_ID)--add by jchl 2014-08-21 
                then 
                ...
                NUM,DOSAGE,case when  ISNUMERIC (LEFT(A.UNIT,1))>0 then '['+A.UNIT +']' else  A.UNIT end UNIT,ORDER_SPEC,BOOK_DATE,ORDER_BDATE,FIRST_TIMES,
			 ORDER_USAGE,
			 case 
...
union all 
select ORDER_ID,GROUP_ID,INPATIENT_ID,DEPT_ID,WARD_ID,MNGTYPE,NTYPE,ORDER_DOC,HOITEM_ID,ITEM_CODE,XMLY,
--case when (@cfg7177=0 or XMLY=2 ) then  ORDER_CONTEXT else substring(ORDER_SPEC,1,CHARINDEX('%',ORDER_SPEC))+ORDER_CONTEXT   end ORDER_CONTEXT,
   case when (@cfg7177=0 or XMLY=2 )
              -- or ORDER_USAGE like '%皮试%'--add by 岳成成 2014-07-29
               --(select top 1 YPSPMBZ  from VI_YP_YPCD d where d.cjid=a.HOITEM_ID)--add by jchl 2014-08-21
                then 
				 case when CHARINDEX( '【取消】',ORDER_CONTEXT)>0
					 then
			...
			jldwlx,ssmz_print,DEL_PRINT,MEMO,MEMO1,MEMO2,IsprintYPGG,ORDER_EDATE 
			,WZX_FLAG ,AUDITING_USER,AUDITING_USER1, ORDER_EDOC ,ORDER_EUSER,ORDER_EUSER1 ,0 tcid
			from  ZY_ORDERRECORD_H A where INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID and mngtype in(0,1,5) and delete_bit=0 ) a  
            ORDER BY ORDER_BDATE,GROUP_ID,SERIAL_NO,ORDER_ID  
            
              update #temporder set   ORDER_CONTEXT=REPLACE(ORDER_CONTEXT,'[高危药品]',@cfg7196)   where  CHARINDEX( '[高危药品]',ORDER_CONTEXT)>0
          --update ZY_LOGORDERPRT set   ORDER_CONTEXT=REPLACE(ORDER_CONTEXT,'【取消】','')+'【取消】'  where  CHARINDEX( '【取消】',ORDER_CONTEXT)>0  
          --and  INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID  
            
          --update ZY_TMPORDERPRT set   ORDER_CONTEXT=REPLACE(ORDER_CONTEXT,'【取消】','')+'【取消】'  where  CHARINDEX( '【取消】',ORDER_CONTEXT)>0  
          --and  INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID  
  ...
  begin
           ...
           declare @tctcid bigint 
            ...
            begin
			 ...
			 (case 
			 when WZX_FLAG>0 
			 then '【取消】'-- MEMO2 Modify by jchl
			 else '' end )+(select top 1 ITEM_NAME from JC_HSITEM where ITEM_ID=d.SUBITEM_ID)  ORDER_CONTEXT,a.NUM*c.NUM*d.NUM,
...
BEGIN  
 ...
 BEGIN    
  ...
  IF(SELECT COUNT(*)  
     FROM  #temporder  
     WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID AND MNGTYPE=0 AND STATUS_FLAG IN (0)   
    AND (DEPT_ID NOT IN (SELECT DEPTID FROM SS_DEPT) OR SSMZ_PRINT=1) and DEPT_ID<>@jy_dept  and dept_id not in (@cfg7126) AND DELETE_BIT=0 AND DEL_PRINT=0 --ADD BY TANY 2011-01-21 DEL_PRINT=0 
                         AND ORDER_BDATE < (SELECT MAX(ORDER_BDATE) AS ORDER_ID FROM #temporder  
                           WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID AND MNGTYPE=0 AND STATUS_FLAG NOT IN (0,9)   
                                               AND (DEPT_ID NOT IN (SELECT DEPTID FROM SS_DEPT) OR SSMZ_PRINT=1 ) and DEPT_ID<>@jy_dept and dept_id not in (@cfg7126)  AND DELETE_BIT=0 AND DEL_PRINT=0 --ADD BY TANY 2011-01-21 DEL_PRINT=0  
                                               AND ORDER_ID NOT IN (SELECT ORDER_ID FROM ZY_LOGORDERPRT WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID)) )<>0  
  BEGIN  
   SET @OUTCODE=-1  
   SET @OUTMSG='该病人有未发送的长期医嘱，请发送后再打印！'  
  ...
  WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID AND MNGTYPE=0 AND STATUS_FLAG NOT IN (0,9) AND (DEPT_ID NOT IN (SELECT DEPTID FROM SS_DEPT) OR SSMZ_PRINT=1 ) and DEPT_ID<>@jy_dept and dept_id not in (@cfg7126)   AND  
  ...
  SELECT INPATIENT_ID,BABY_ID,ORDER_ID,GROUP_ID,ORDER_BDATE,NTYPE,LTRIM(RTRIM(ORDER_CONTEXT))+(case when a.WZX_FLAG>0 and @cfg7138='1' then '【原因：'+memo2+'】' else '' end)    ORDER_CONTEXT,NUM,DOSAGE,UNIT,case when isprintypgg is null or isprintypgg=0 then '' else LTRIM(RTRIM(ORDER_SPEC)) end ORDER_SPEC, --modify by zouchihua 2011-11-30 规格是否打印
   CASE B.IS_PRINT 
   WHEN 0 
   THEN case when LTRIM(RTRIM(isnull(printname,'')))='' then ORDER_USAGE else  printname end --LTRIM(RTRIM(isnull(B.printname, ORDER_USAGE)))
   +CASE WHEN PS_FLAG=1 THEN ' (-)' WHEN PS_FLAG=2 THEN ' (+)' WHEN PS_FLAG=21 THEN ' (++)' WHEN PS_FLAG=22 THEN ' (+++)' ELSE '' END 
   ELSE '' END ORDER_USAGE,  
   FREQUENCY,ORDER_DOC,AUDITING_USER,AUDITING_USER1,ORDER_EDATE,ORDER_EDOC,ORDER_EUSER,ORDER_EUSER1,STATUS_FLAG,SERIAL_NO,memo2,dropsper,tcid  --滴度 
  ...
  SELECT INPATIENT_ID,BABY_ID,ORDER_ID ORDER_ID,GROUP_ID,ORDER_BDATE ORDER_BDATE,NTYPE,case when wzx_flag>0 then  '【取消】中草药处方'+(case when a.WZX_FLAG>0 and @cfg7138='1' then '【原因：'+memo2+'】' else '' end) 
   ...
   ORDER_EDATE ORDER_EDATE,ORDER_EDOC,ORDER_EUSER,ORDER_EUSER1,STATUS_FLAG,0 SERIAL_NO,memo2,dropsper ,tcid --滴度 
  FROM #temporder  a
  WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID AND MNGTYPE=0 AND STATUS_FLAG NOT IN (0,9) AND (DEPT_ID NOT IN (SELECT DEPTID FROM SS_DEPT) OR SSMZ_PRINT=1 ) and DEPT_ID<>@jy_dept and dept_id not in (@cfg7126) AND  
   ORDER_ID NOT IN (SELECT ORDER_ID FROM ZY_LOGORDERPRT WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID) AND  
   GROUP_ID NOT IN (SELECT GROUP_ID FROM ZY_LOGORDERPRT WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID GROUP BY GROUP_ID) AND  
...
--   ORDER_DOC,AUDITING_USER,AUDITING_USER1,ORDER_EDOC,ORDER_EUSER,ORDER_EUSER1,STATUS_FLAG  
  ORDER BY ORDER_BDATE,GROUP_ID,SERIAL_NO,ORDER_ID  
	  ...
	  SELECT INPATIENT_ID,BABY_ID,ORDER_ID,GROUP_ID,ORDER_BDATE,NTYPE,LTRIM(RTRIM(ORDER_CONTEXT))+(case when a.WZX_FLAG>0 and @cfg7138='1' then '【原因：'+memo2+'】' else '' end) ORDER_CONTEXT,NUM,DOSAGE,UNIT,case when isprintypgg is null or isprintypgg=0 then '' else LTRIM(RTRIM(ORDER_SPEC)) end ORDER_SPEC, --modify by zouchihua 2011-11-30 规格是否打印
	   ...
	   +CASE WHEN PS_FLAG=1 THEN ' (-)' WHEN PS_FLAG=2 THEN ' (+)' WHEN PS_FLAG=21 THEN ' (++)' WHEN PS_FLAG=22 THEN ' (+++)' ELSE '' END ELSE '' END ORDER_USAGE,  
	   FREQUENCY,ORDER_DOC,AUDITING_USER,AUDITING_USER1,ORDER_EDATE,ORDER_EDOC,ORDER_EUSER,ORDER_EUSER1,STATUS_FLAG,SERIAL_NO,memo2,dropsper ,tcid --滴度 
	  FROM #temporder A LEFT JOIN(SELECT NAME,IS_PRINT,printname FROM JC_USAGEDICTION WHERE NAME <> '') B ON A.ORDER_USAGE=B.NAME
	  WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID AND MNGTYPE=0 AND STATUS_FLAG NOT IN (0,9) AND (DEPT_ID NOT IN (SELECT DEPTID FROM SS_DEPT) OR SSMZ_PRINT=1 ) and DEPT_ID<>@jy_dept and dept_id not in (@cfg7126) AND  
	   ORDER_ID NOT IN (SELECT ORDER_ID FROM ZY_LOGORDERPRT WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID) AND  
  ...
  INTO @CS_INPATIENT_ID,@CS_BABY_ID,@CS_ORDER_ID,@CS_GROUP_ID,  
   ...
   @CS_ORDER_EUSER,@CS_ORDER_EUSER1,@CS_STATUS_FLAG,@CS_SERIAL_NO,@CS_CancelMemo,@CS_DROPSPER,  --滴速
   @cs_tcid--套餐
...
--------------------------------------转科医嘱换页处理开始---------------------------------------
 declare @Sccqflag int
  ...
  ------------------------------------转科医嘱换页处理结束------------------------------------   
    ...
    WHILE @K <= @ROWCOUNT   
     ...
     BEGIN  
      INSERT INTO ZY_LOGORDERPRT  
       ...
       @CS_NUM,@CS_DOSAGE,@CS_UNIT,@CS_ORDER_SPEC,@CS_ORDER_USAGE,@CS_FREQUENCY,@CS_ORDER_DOC,@CS_AUDITING_USER,@CS_AUDITING_USER1,@CS_ORDER_EDATE,  
       @CS_ORDER_EDOC,@CS_ORDER_EUSER,@CS_ORDER_EUSER1,@USER_ID,@CS_CancelMemo,@CS_DROPSPER)  
     END  
     ELSE --折行前面加空行  
       --改变折行的组标志  
   ...
   BEGIN  
    ...
    VALUES (@CS_INPATIENT_ID,@CS_BABY_ID,@MAXPAGENO,@MAXROWNO,@PAGESTATUS,@ROWSTATUS,@GROUPSTATUS,@PRTSTATUS,  
     ...
     @CS_ORDER_USAGE,@CS_FREQUENCY,@CS_ORDER_DOC,@CS_AUDITING_USER,@CS_AUDITING_USER1,@CS_ORDER_EDATE,@CS_ORDER_EDOC,@CS_ORDER_EUSER,@CS_ORDER_EUSER1,@USER_ID,@CS_CancelMemo,@CS_DROPSPER)  
   ...
   FETCH NEXT FROM T1   
    ...
    @CS_FREQUENCY,@CS_ORDER_DOC,@CS_AUDITING_USER,  
    @CS_AUDITING_USER1,@CS_ORDER_EDATE,@CS_ORDER_EDOC,  
    @CS_ORDER_EUSER,@CS_ORDER_EUSER1,@CS_STATUS_FLAG,@CS_SERIAL_NO,@CS_CancelMemo,@CS_DROPSPER,@cs_tcid
  ...
  WHILE @@FETCH_STATUS = 0  
  BEGIN  
	  if exists(select 1 from JC_CONFIG where ID=6217 and CONFIG='1')
	  begin
	   ...
	   WHERE INPATIENT_ID=@CS_INPATIENT_ID AND BABY_ID=@CS_BABY_ID AND ORDER_ID=@CS_ORDER_ID AND GROUP_ID=@CS_GROUP_ID AND  
		PRT_STATUS NOT IN (0,-1,3)  and  (  isnull(MEMO,'')!='取消')--取消的不再更新 
	  end
	  else
	  begin 
	   UPDATE ZY_LOGORDERPRT SET ORDER_EDATE=@CS_ORDER_EDATE,ORDER_EDOC=@CS_ORDER_EDOC,ORDER_EUSER=@CS_ORDER_EUSER,  
...
,DATEPART(DD,ORDER_EDATE))) END END AS EDATE,  
   ...
   CASE LTRIM(RTRIM(ORDER_CONTEXT)) WHEN '┃' THEN ' ' ELSE DBO.FUN_ZY_SEEKEMPLOYEENAME(CASE WHEN ORDER_EDOC IS NULL THEN -1 ELSE ORDER_EDOC END) END ORDER_EDOC,  
   CASE LTRIM(RTRIM(ORDER_CONTEXT)) WHEN '┃' THEN ' ' ELSE DBO.FUN_ZY_SEEKEMPLOYEENAME(CASE WHEN ORDER_EUSER IS NULL THEN -1 ELSE ORDER_EUSER END) END ORDER_EUSER,  
   ORDER_ID,prt_status,pageno,rowno,MEMO --add by zouchihua 2011-12-29   add memo  by jchl
  FROM ZY_LOGORDERPRT  
  WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID  
  ORDER BY PAGENO,ROWNO   
 ...
 BEGIN  
  ...
  IF(SELECT COUNT(*)  
     FROM #temporder  
     WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID AND MNGTYPE IN (1,5) AND STATUS_FLAG IN (0) AND (DEPT_ID NOT IN (SELECT DEPTID FROM SS_DEPT) OR SSMZ_PRINT=1 ) and DEPT_ID<>@jy_dept and dept_id not in (@cfg7126) AND DELETE_BIT=0 AND DEL_PRINT=0 --ADD BY TANY 2011-01-21 DEL_PRINT=0 
		AND ORDER_BDATE < (SELECT MAX(ORDER_BDATE) AS ORDER_ID FROM #temporder  
                                       WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID AND MNGTYPE IN (1,5) AND STATUS_FLAG NOT IN (0,9) AND (DEPT_ID NOT IN (SELECT DEPTID FROM SS_DEPT) OR SSMZ_PRINT=1) and DEPT_ID<>@jy_dept and dept_id not in (@cfg7126) AND  
                               ORDER_ID NOT IN (SELECT ORDER_ID FROM ZY_TMPORDERPRT WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID)) )<>0  
  BEGIN  
   SET @OUTCODE=-1  
   SET @OUTMSG='该病人有未发送的临时医嘱，请发送后再打印！'  
...
IF(SELECT COUNT(*)  
  ...
  --                             ORDER_ID NOT IN (SELECT ORDER_ID FROM ZY_TMPORDERPRT WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID))
    ...
    FROM #temporder  
     ...
     and NTYPE in(1,2) and PS_FLAG=0  and XMLY=2 and ORDER_USAGE='皮试' and    wzx_flag<0
  --   AND (DEPT_ID NOT IN (SELECT DEPTID FROM SS_DEPT) OR SSMZ_PRINT=1 ) and DEPT_ID<>@jy_dept and dept_id not in (@cfg7126) AND DELETE_BIT=0 AND DEL_PRINT=0 --ADD BY TANY 2011-01-21 DEL_PRINT=0 
		--AND ORDER_BDATE < (SELECT MAX(ORDER_BDATE) AS ORDER_ID FROM #temporder  
  --                                     WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID AND MNGTYPE IN (1,5) AND STATUS_FLAG NOT IN (0,9) AND (DEPT_ID NOT IN (SELECT DEPTID FROM SS_DEPT) OR SSMZ_PRINT=1) and DEPT_ID<>@jy_dept and dept_id not in (@cfg7126) AND  
  --                             ORDER_ID NOT IN (SELECT ORDER_ID FROM ZY_TMPORDERPRT WHERE INPATIENT_ID=@INPATIENT_ID AND BABY_ID=@BABY_ID)) 
...
-- select order_id,group_id into #temp_cy_order from #temporder  where 1=2--获得临时表结构
  ...
  set @flagcount=0;
  DECLARE Tt2 CURSOR FOR    select order_id,GROUP_ID from #temporder a
   WHERE A.INPATIENT_ID=@INPATIENT_ID AND A.BABY_ID=@BABY_ID AND A.MNGTYPE IN (1,5) AND (DEPT_ID NOT IN (SELECT DEPTID FROM SS_DEPT) OR SSMZ_PRINT=1 ) and DEPT_ID<>@jy_dept and dept_id not in (@cfg7126) AND STATUS_FLAG NOT IN (0,9)  
  ...
  SELECT A.INPATIENT_ID,A.BABY_ID,A.ORDER_ID ORDER_ID,A.GROUP_ID,A.ORDER_BDATE,NTYPE,  
   ...
   +CASE WHEN PS_FLAG=1 THEN ' (-)' WHEN PS_FLAG=2 THEN ' (+)' WHEN PS_FLAG=21 THEN ' (++)' WHEN PS_FLAG=22 THEN ' (+++)' ELSE '' END ELSE '' END+CASE MNGTYPE WHEN 5 THEN @mryf ELSE '' END ORDER_USAGE,  
   LTRIM(RTRIM(A.FREQUENCY)) FREQUENCY,A.ORDER_DOC,A.AUDITING_USER,A.AUDITING_USER1,0 EXEUSER,A.ORDER_EDATE EXECDATE,A.STATUS_FLAG,SERIAL_NO,memo2,DROPSPER --滴速 
   ,A.zsl,A.zsldw,tcid 
  ...
  '中草药处方' +case when @zcyxsmc='0' then '' else  (case when rtrim(isnull(MEMO,''))!=''   then '【'+isnull(MEMO,'')+'】' else isnull((select top 1 '【'+MBMC+'】' from  jc_cfmb where MBXH=A.PS_ORDERID),'') end  )   end
   ...
   LTRIM(RTRIM(A.FREQUENCY)) FREQUENCY,A.ORDER_DOC,A.AUDITING_USER,A.AUDITING_USER1,0 EXEUSER,/*MIN(B.EXECDATE)*/A.ORDER_EDATE EXECDATE,A.STATUS_FLAG,0 SERIAL_NO,memo2,DROPSPER --滴速
   ,A.zsl,A.zsldw ,tcid 
...
--   A.ORDER_DOC,A.AUDITING_USER,A.AUDITING_USER1/*,B.EXEUSER*/,A.STATUS_FLAG,A.MNGTYPE  
  ...
  INTO @CS_INPATIENT_ID,@CS_BABY_ID,@CS_ORDER_ID,@CS_GROUP_ID,  
   ...
   @CS_DOSAGE,@CS_UNIT,@CS_ORDER_SPEC,@CS_ORDER_USAGE,  
   @CS_FREQUENCY,@CS_ORDER_DOC,@CS_AUDITING_USER,  
   @CS_AUDITING_USER1,@CS_EXEUSER,@CS_EXECDATE,@CS_STATUS_FLAG,@CS_SERIAL_NO,@CS_CancelMemo,@CS_DROPSPER --滴速  
   ,@zl,@zldw,@cs_tcid
  ...
  BEGIN  
   ...
   IF @TMPROWS-@MAXROWNO+1< @ROWCOUNTS and (case when @cs7136='0' then @CS_NTYPE else 1 end )<>3--  组占的行数  @ROWS-@MAXROWNO+1 < @ROWCOUNT
    ...
    --IF @ROWS-@L
     --THEN 
     begin 
      SET @LASTROWS=@TMPROWS-@MAXROWNO+1--@LOGROWS-@MAXROWNO+1  Modify By Tany 2014-09-13 应该@TMPROWS
      ...
      -- @CS_ORDER_ID,0,@CS_ORDER_BDATE,null,case when @cfg7173='' then '      ┃' else  @cfg7173 end, 
       ...
       SET @J=@J+1  
       --换行换页  
         IF @MAXROWNO=@TMPROWS--@LOGROWS--@ROWS --Modify By Tany 2014-09-13 应该@TMPROWS
         begin  
           --THEN  
               SET @MAXPAGENO=@MAXPAGENO+1  
               SET @MAXROWNO=1  
          ...
          ELSE IF @TMPROWS=@MAXROWNO+1--@LOGROWS=@MAXROWNO+1 Modify By Tany 2014-09-13 应该@TMPROWS --@ROWS=@MAXROWNO+1  
           --THEN 
           begin 
                 SET @MAXROWNO=@MAXROWNO+1  
                 SET @PAGESTATUS=2 
  ...
  --插入数据  
    ...
    BEGIN  
     ...
     BEGIN  
      ...
      VALUES (@CS_INPATIENT_ID,@CS_BABY_ID,@MAXPAGENO,@MAXROWNO,@PAGESTATUS,@ROWSTATUS,@GROUPSTATUS,@PRTSTATUS,  
       ...
       @USER_ID,@CS_CancelMemo,@CS_DROPSPER,0,@zl,@zldw)  
     END  
     ELSE --折行前面加空行  
     BEGIN  
      --改变折行的组标志  
   ...
   BEGIN  
    ...
    VALUES (@CS_INPATIENT_ID,@CS_BABY_ID,@MAXPAGENO,@MAXROWNO,@PAGESTATUS,@ROWSTATUS,@GROUPSTATUS,@PRTSTATUS,  
     @CS_ORDER_ID,@CS_GROUP_ID,@CS_ORDER_BDATE,@CS_NTYPE,LTRIM(RTRIM(@CS_ORDER_CONTEXT)),@CS_NUM,@CS_DOSAGE,@CS_UNIT,@CS_ORDER_SPEC,  
     @CS_ORDER_USAGE,@CS_FREQUENCY,@CS_ORDER_DOC,@CS_AUDITING_USER,@CS_AUDITING_USER,@CS_EXEUSER,@CS_EXECDATE,@USER_ID,@CS_CancelMemo,@CS_DROPSPER,0
     ,@zl,@zldw)  
    --换行换页  
    IF @MAXROWNO=@TMPROWS  
    BEGIN  
  ...
  ------------------------------------------------------------------------  
   ...
   INTO @CS_INPATIENT_ID,@CS_BABY_ID,@CS_ORDER_ID,@CS_GROUP_ID,  
    ...
    @CS_DOSAGE,@CS_UNIT,@CS_ORDER_SPEC,@CS_ORDER_USAGE,  
    @CS_FREQUENCY,@CS_ORDER_DOC,@CS_AUDITING_USER,  
    @CS_AUDITING_USER1,@CS_EXEUSER,@CS_EXECDATE,@CS_STATUS_FLAG,@CS_SERIAL_NO ,@CS_CancelMemo,@CS_DROPSPER ,@zl,@zldw,@cs_tcid
  ...
  WHILE @@FETCH_STATUS = 0  
  BEGIN  
   UPDATE ZY_TMPORDERPRT SET EXECDATE=@CS_REALEXECDATE,EXEUSER=@CS_REALEXEUSER,
   exec_duser=@CS_REALEXEUSER1,  PRT_STATUS=2,
   AUDITING_USER=ISNULL(AUDITING_USER,@CS_REALEXEUSER) --Modify By Tany 2015-06-03 如果没有转抄护士，则填实际执行护士
   ...
   UPDATE ZY_TMPORDERPRT SET EXECDATE=@CS_REALEXECDATE,EXEUSER=@CS_REALEXEUSER,
   exec_duser=@CS_REALEXEUSER1,PRT_STATUS=-1,
   AUDITING_USER=ISNULL(AUDITING_USER,@CS_REALEXEUSER) --Modify By Tany 2015-06-03 如果没有转抄护士，则填实际执行护士  
  ...
  SELECT PAGENO,ROWNO,PAGE_STATUS,ROW_STATUS,PRT_STATUS,ORDER_ID,  
         ...
         CASE LTRIM(RTRIM(ORDER_CONTEXT)) WHEN '┃' THEN ' ' ELSE DBO.FUN_ZY_SEEKEMPLOYEENAME(CASE WHEN ORDER_EDOC IS NULL
          ...
          THEN -1 ELSE ORDER_EDOC END) END ORDER_EDOC,  
            ...
            CASE WHEN LTRIM(RTRIM(ORDER_CONTEXT))='┃' OR ROW_STATUS=2 /*OR (NTYPE NOT IN (1,2,3) AND UPPER(FREQUENCY)='QD')*/ THEN ' ' ELSE CASE WHEN (GROUP_STATUS=0 OR GROUP_STATUS=1) THEN DROPSPER ELSE ' ' END END AS DROPSPER  --滴速 
            ,GROUP_ID ,
            ORDER_SPEC,MEMO --药品规格 add memo by jchl 
            ,case when ntype in(1,2,3) then 1 else 2 end xmly
  FROM ZY_LOGORDERPRT  
  ...
  ORDER_SPEC,MEMO --药品规格 add memo by jchl
   ...
   SELECT PAGENO,ROWNO,PAGE_STATUS,ROW_STATUS,PRT_STATUS,ORDER_ID,  
      ...
      '' ETIME,  --执行时间与开遗嘱时间不同时。要打印日期--SUBSTRING(CONVERT(CHAR,EXECDATE,108),1,5) END END AS ETIME,  
       ...
       '【取消】'  DROPSPER  --滴速 
             ,GROUP_ID  ,
         '' AS ORDER_SPEC,MEMO --药品规格 add memo by jchl
             ,case when ntype in(1,2,3) then 1 else 2 end xmly ,
             -- CASE WHEN LTRIM(RTRIM(ORDER_CONTEXT))='┃' OR ROW_STATUS=2 OR NTYPE  not IN (1,2,3)  THEN ' ' WHEN NTYPE=3 THEN LTRIM(RTRIM(CONVERT(CHAR,CONVERT(INT,DOSAGE))))+'付' ELSE LTRIM(RTRIM(CONVERT(CHAR,CAST(zl AS FLOAT))))+LTRIM(RTRIM(zldw)) END AS zldw
             '' zldw --Modify by zouchihua 2013-5-30 总量单位不应该打印
           --增加了总量打印 add by zouchihua 2012-12-13
