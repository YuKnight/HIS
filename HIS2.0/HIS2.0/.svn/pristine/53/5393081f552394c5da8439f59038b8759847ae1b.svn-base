	...
	public struct MenuTag
	{
        ...
        public static MenuTag GetTag(string dllName,string functionName,string functionTag,string name,RelationalDatabase database)
        {
            ...
            if ( row != null )
            {
                ...
                tag.Jgbm = Convert.ToInt32( row["jgbm"] ) == -1 ? TrasenFrame.Forms.FrmMdiMain.Jgbm : Convert.ToInt32( row["jgbm"] );//Add By Tany 2010-01-14 Modify By Tany 如果没有设置机构编码，则默认机构编码设置为登录机构编码
