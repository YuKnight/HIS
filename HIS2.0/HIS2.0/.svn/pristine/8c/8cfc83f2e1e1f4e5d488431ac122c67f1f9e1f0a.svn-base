...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_ZYHS_Pivas_SELECTFEE]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_ZYHS_Pivas_SELECTFEE]
GO

/****** Object:  StoredProcedure [dbo].[SP_ZYHS_Pivas_SELECTFEE]    Script Date: 03/19/2015 20:05:42 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[SP_ZYHS_Pivas_SELECTFEE]
(
	@BINID varchar(20), 
	@BABYID BIGINT, 
	@WARDDEPTID BIGINT,
...
SELECT DBO.FUN_GETDATE(EXECDATE) 医嘱日期,  
	LTRIM(RTRIM(A.ITEM_NAME)) + CASE WHEN A.TLFS=1 THEN  '【缺药】' ELSE '' END 
	+ CASE WHEN A.DELETE_BIT=1 THEN  '【已删除】' ELSE '' END 
	+ CASE WHEN A.BZ IS NOT NULL THEN LTRIM(RTRIM(A.BZ)) ELSE '' END AS 内容,
	A.GG 规格,CASE WHEN A.CZ_FLAG IN (2,3) THEN A.NUM ELSE A.NUM/A.ISANALYZED END  数量,
	...
	C.NAME 发送护士,D.NAME 记账员,A.ISJSY 基数药,B.ISJZ,A.JZ_FLAG,A.DISCHARGE_BIT,LTRIM(RTRIM(A.ITEM_NAME)) AS 名称, 
	...
	DBO.FUN_GETEMPNAME(BOOK_USER) 操作人,FEEBOOK_DATE 操作时间,TYPE,ORDEREXEC_ID,STATITEM_CODE,A.is_PvsScn,A.pvs_xh 频次,
...
FROM 
	(
		SELECT C.EXECDATE,A.ITEM_NAME, A.GG ,    
			...
			CZ_FLAG,D.ORDER_ID,A.ID,A.CHARGE_BIT,A.FINISH_BIT,D.BOOK_DATE as book_date1 ,
			...
			BOOK_USER,A.BOOK_DATE FEEBOOK_DATE ,ORDEREXEC_ID,STATITEM_CODE,is_PvsScn,a.pvs_xh,A.pvs_zdb
		FROM 
			(
				SELECT A.ID, PRESCRIPTION_ID,ORDEREXEC_ID, XMLY,XMID, NUM, 
					...
					DELETE_BIT,DISCHARGE_BIT,TYPE,A.BZ,CASE WHEN K.CJID IS NULL THEN 0 ELSE 1 END ISJSY,is_PvsScn,a.pvs_xh,A.pvs_zdb,
...
--				LEFT JOIN YP_YPJX C ON B.YPJX=C.ID 
				...
				WHERE A.XMLY=1 AND exists(select 1 from ZY_INPATIENT t where INPATIENT_NO=@BINID and t.INPATIENT_ID=A.INPATIENT_ID) 
					--and INPATIENT_ID=@BINID 
					AND BABY_ID=  @BABYID  
					and datediff(Day,@strDate,A.PRESC_DATE) >=0 
					and datediff(Day,@endDate,A.PRESC_DATE) <=0 
					and is_PvsScn=1		--已扫描
			) A , 
			ZY_ORDEREXEC C , 
			(
				...
				FROM ZY_ORDERRECORD t
				WHERE exists(select 1 from ZY_INPATIENT t where INPATIENT_NO=@BINID and t.INPATIENT_ID=t.INPATIENT_ID) 
					AND BABY_ID=  @BABYID  
			) D  
		WHERE A.ORDEREXEC_ID=C.ID AND C.ORDER_ID=D.ORDER_ID AND A.DELETE_BIT=0
	) A  
...
ORDER BY EXECDATE,TYPE,BOOK_DATE,CZ_FLAG,pvs_xh
