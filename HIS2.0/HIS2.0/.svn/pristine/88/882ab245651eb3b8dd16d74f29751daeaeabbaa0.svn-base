    ...
    public class mzys_zyz
    {
        ...
        public  mzys_zyz(Guid zyzid, RelationalDatabase database)
        {
            ...
            if (tb.Rows.Count != 0)
            {
                ...
                _mzh = Convertor.IsNull(tb.Rows[0]["mzh"], "");
            }
        ...
        {
            string ssql = @"select '' 序号,brxm 姓名,dbo.FUN_ZY_SEEKSEXNAME(xb) 性别,dbo.fun_zy_age(csrq,3,getdate()) 年龄,mzzdmc 门诊诊断,
dbo.fun_getdeptname(dryks) 入院科室,jyyj 建议押金,c.name 入院途径,bz 备注,BRYBZ 已入院,rydjsj 入院登记时间,b.id,mzh 门诊号
from YY_BRXX a  
inner join mzys_zyzdj b on a.brxxid=b.brxxid 
left join jc_rytj c on b.rytj=c.id 
left join yy_kdjb d on a.brxxid= d.brxxid and zfbz=0 
where  1=1 ";
            //150204  chencan 过滤过期的住院证
            SystemCfg cfg5152 = new SystemCfg(5152);
            if (!String.IsNullOrEmpty(cfg5152.Config))
            {
                try
                {
                    ssql += string.Format(" and b.djsj+{0}>=getdate()", Int32.Parse(cfg5152.Config));
                }
                catch { }
            }
            if (rq1 != "")
        ...
        public void Save(out Guid newid, out int err_code,out string err_text, RelationalDatabase _DataBase)
        {
            try
            {
                ...
                if (_mzh == null) _mzh = "";
                parameters[16].Text = "@mzh"; //Modify By zp 2013-10-22
                parameters[16].Value = _mzh;
        ...
        public void Delete(Guid djid, RelationalDatabase _DataBase)
        {
            ...
            int ncount = _DataBase.DoCommand(ssql);
            if (ncount != 1) throw new Exception("住院证已经作废。");
        }
