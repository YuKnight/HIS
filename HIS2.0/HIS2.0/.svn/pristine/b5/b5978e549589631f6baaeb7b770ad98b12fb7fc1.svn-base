	...
	public class Frmjxcqkb : System.Windows.Forms.Form
	{
		...
		private void buttj_Click(object sender, System.EventArgs e)
		{
			try
			{
                ...
                parameters[4].Text = "@deptid_ck";
                parameters[4].Value = Convert.ToInt32(Convertor.IsNull(cmbck.SelectedValue, "0"));

                DataTable tb;
                if (checkBox2.Checked==true)
                    tb=InstanceForm.BDatabase.GetDataTable("SP_YK_tj_jxcqkb_pfje", parameters, 30);//进货金额
                else
                    tb = InstanceForm.BDatabase.GetDataTable("SP_YK_tj_jxcqkb", parameters, 30);
				tb.TableName="Tb";
		...
		private void butprint_Click(object sender, System.EventArgs e)
		{
			try
			{
                ...
                string kjqj="";
                string bz = "";
                string ckmc = cmbyjks.Text.Trim() != cmbck.Text.Trim() ? cmbck.Text.Trim() : "";
                if (checkBox1.Checked == true)
                {
                    kjqj = "仓库名称:" + ckmc+"\r";
                    kjqj = kjqj+"年份:" + cmbyear.Text + "年  月份:" + cmbmonth.Text + "月  日期区间" + this.statusBarPanel3.Text + "  药品类型:" + cmbyplx.Text;
                    bz="进销存月报表";
                }
                else
                {
                    kjqj = "仓库名称:" + ckmc + "\r";
                    kjqj =kjqj+ "年份:" + cmbyear.Text + "年    药品类型:" + cmbyplx.Text;
                    bz="进销存年报表";
                }
