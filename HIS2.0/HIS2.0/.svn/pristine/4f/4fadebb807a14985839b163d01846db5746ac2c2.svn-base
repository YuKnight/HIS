IF EXISTS (SELECT name 
	   FROM   sysobjects 
	   WHERE  name = N'SP_YF_TJ_PJRXSLTJ' 
	   AND 	  type = 'P')
    DROP PROCEDURE SP_YF_TJ_PJRXSLTJ
GO
CREATE PROCEDURE SP_YF_TJ_PJRXSLTJ
 (
   @dtp1 varchar(30),
   @dtp2 varchar(30),
   @yplx int,
...
begin
	...
	set @count=DATEDIFF(day,@dtp1,@dtp2)+1;
	 ...
	 'c.s_ypdw 单位,c.cjid, '+
	 '1 ydwbl,c.ypdw nypdw,cast(1 as smallint) 更新 from vi_YF_dj a,vi_YF_djmx b,vi_yp_ypcd c  '+
	 'where a.id=b.djid and b.cjid=c.cjid  and a.deptid='+cast(@deptid as char(10))+' and '+
	 ' a.ywlx in(''017'',''018'',''020'',''021'',''022'',''023'',''025'',''026'') and a.shrq>='''+ @dtp1+''' and a.shrq<='''+@dtp2+''' and a.shbz=1 '
