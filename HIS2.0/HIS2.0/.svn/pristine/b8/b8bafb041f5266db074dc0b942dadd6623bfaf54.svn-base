...
namespace ts_PrintProcess
{
    ...
    public class WorkStaticFun
    {
        ...
        public static void SendMessage(MessageInfo message)
        {
            ...
            RelationalDatabase database = FrmMdiMain.Database.GetCopy();
            try
            {
                database.Open();
                ...
                //是否启用系统消息,否的话，不发送
                if (Convert.ToInt32(new SystemCfg(0009).Config) != 0)
                {
                    return;
                }
                //Add By Tany 2009-12-24
                ...
                FrmMdiMain.WriteFrameLocalLog(new string[] { sql }, true);
            ...
            {
                FrmMdiMain.WriteFrameLocalLog(new string[] { error.Message }, true);
            }
            finally
            {
                database.Close();
        ...
        {
            string ip = string.Empty; //PubStaticFun.GetIPAddress();
            string mac = string.Empty;  //PubStaticFun.GetMacAddress();
