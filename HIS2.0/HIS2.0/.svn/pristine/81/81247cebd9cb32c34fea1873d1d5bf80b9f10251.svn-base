...
begin
 ...
 if @jhjetj=0
		begin
			select a.CJID 编码,ywjc as 业务类型,S_YPPM 品名,S_YPGG 规格,S_YPDW 单位,
			(case when a.YWLX='005' then round(sum(a.LSJE)/sum(YPSL),4) else a.LSJ end) 单价,
			case when ywjc = '调整减少' then  -cast(SUM(YPSL) as float) else cast(SUM(YPSL) as float) end 数量, 
			case when ywjc = '调整减少' then  -sum(lsje) else sum(lsje) end 金额 
		 
			from YP_TJ_YMJCMX a inner join #tempyjjl b on a.YJID=b.yjid inner join #ywlx c on a.YWLX=c.ywlx
			inner join yp_ypcjd D on a.CJID=d.CJID where  tjlx='本期其它'  and LSJE<>0     and a.YWLX<>'005'
			group by a.CJID,d.SHH,S_YPPM,S_YPGG,S_YPDW,a.LSJ,a.YWLX,ywjc 
			union all 
			select  a.CJID 编码,ywjc as 业务类型,S_YPPM 品名,S_YPGG 规格,S_YPDW 单位,
			round(sum(a.LSJE)/sum(YPSL/YDWBL),4)  单价,cast(sum(YPSL/YDWBL) as float) 数量, 
			sum(LSJE) 金额
			from YP_TJ_YMJCMX a inner join #tempyjjl b on a.YJID=b.yjid inner join #ywlx c on a.YWLX=c.ywlx
			inner join yp_ypcjd D on a.CJID=d.CJID where  tjlx='本期其它'    and LSJE<>0    and a.YWLX='005'
		...
		begin
			select  a.CJID 编码,ywjc as 业务类型,S_YPPM 品名,S_YPGG 规格,YPDW 单位,
			a.JHJ 单价,
			case when ywjc = '调整减少' then  -cast(SUM(YPSL) as float) else cast(SUM(YPSL) as float) end 数量, 
			case when ywjc = '调整减少' then  -sum(JHJE) else sum(JHJE) end 金额 			 
			from YP_TJ_YMJCMX a inner join #tempyjjl b on a.YJID=b.yjid inner join #ywlx c on a.YWLX=c.ywlx
			inner join yp_ypcjd D on a.CJID=d.CJID where  tjlx='本期其它'    and JHJE<>0    and a.YWLX<>'005'
			group by a.CJID,d.SHH,S_YPPM,S_YPGG,YPDW,a.JHJ,a.YWLX,ywjc
			union all 
			select  a.CJID 编码,ywjc as 业务类型,S_YPPM 品名,S_YPGG 规格,S_YPDW 单位,
			round(sum(a.JHJE)/sum(YPSL/YDWBL),4)  单价,cast(sum(YPSL/YDWBL) as float) 数量, 
			sum(JHJE) 金额
			from YP_TJ_YMJCMX a inner join #tempyjjl b on a.YJID=b.yjid inner join #ywlx c on a.YWLX=c.ywlx
			inner join yp_ypcjd D on a.CJID=d.CJID where  tjlx='本期其它'    and JHJE<>0    and a.YWLX='005'
			group by a.CJID,d.SHH,S_YPPM,S_YPGG,S_YPDW,a.YWLX,ywjc
