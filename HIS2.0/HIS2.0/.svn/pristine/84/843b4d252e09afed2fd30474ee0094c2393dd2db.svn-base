IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_yp_tj_jxchzb_sjj]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_yp_tj_jxchzb_sjj]
GO
CREATE PROCEDURE [dbo].[sp_yp_tj_jxchzb_sjj]       
(      
 @yk int,      
 @jhjetj int,      
 @year int,      
...
exec sp_yp_tj_jxchzb_sjj @yk=0,@jhjetj=1,@year=2014,@month=8,@err_code=@err_code output,@err_text=@err_text output      
...
if @yk=1       
 insert into #yjks      
 select  rtrim(KSMC) as xmmc ,deptid from yp_yjks a inner join jc_dept_property b on a.DEPTID=b.DEPT_ID and kslx='药库' and (b.NAME like '%谌家矶%') order by  a.pxxh      
else      
 insert into #yjks      
 select  rtrim(KSMC) as xmmc ,DEPTID from yp_yjks a inner join jc_dept_property b on a.DEPTID=b.DEPT_ID and kslx='药房' and (b.NAME like '%谌家矶%') order by a.pxxh     
