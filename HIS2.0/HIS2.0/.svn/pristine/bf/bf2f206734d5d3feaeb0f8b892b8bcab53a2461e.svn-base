...
namespace ts_yk_bsby
{
	...
	public class Frmypbsby : System.Windows.Forms.Form
	{
		...
		private void csgroupbox2()
		{
            ...
            this.txtpch.Enabled = false; //批次号不可输入
			this.lblxq.Text="";
			this.lblpm.Text="";
			this.lblypmc.Text="";
			this.lblypmc.Tag="0";
...
//			}
		...
		private void Getrow(DataRow row)
		{
            ...
            this.txtpch.Text = row["批次号"].ToString(); //批次号
		...
		private void butadd_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (row["品名"].ToString().Trim()!="")
				{
                    ...
                    {
                        MessageBox.Show( "添加的药品已存在，不能重复添加" , "" , MessageBoxButtons.OK , MessageBoxIcon.Error );
                        return;
                    }
					tb.Rows.Add(row);
					this.Sumje();
		...
		private void butph_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				string[] GrdMappingName={"行号","库存量","单位","进价",  "批次号","kcid",   "批号","效期","库位","kwid","id"};
		...
		private void butprint_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				TrasenFrame.Forms.FrmReportView f=new TrasenFrame.Forms.FrmReportView(Dset.报损报溢单,Constant.ApplicationDirectory+"\\Report\\YK_药品报损报溢单.rpt",parameters,false,str,InstanceForm.BDatabase);
