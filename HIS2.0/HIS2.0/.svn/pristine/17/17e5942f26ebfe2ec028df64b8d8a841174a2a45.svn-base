IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SP_YF_SELECT_MZCF_YFY]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[SP_YF_SELECT_MZCF_YFY]
GO
CREATE PROCEDURE [dbo].[SP_YF_SELECT_MZCF_YFY]      
 (      
...
(case when d.tlfl=''03'' and '+cast(@deptid as varchar(20))+' not in (378,841) and YFMC in(''iv drip'',''H'',''iv'',''im'',''皮下注射'',''iv pump'',''静脉泵入'')  then 0 else cast(b.SL as float) end) 用量,b.JS 剂数,b.DW 单位,    
(case when d.tlfl=''03'' and '+cast(@deptid as varchar(20))+' not in (378,841) and YFMC in(''iv drip'',''H'',''iv'',''im'',''皮下注射'',''iv pump'',''静脉泵入'')   then 0 else cast(round(JE,3) as float) end) 金额,      
...
b.tyid,c.dwbl dwbl      ,
    (case when d.tlfl=''03'' and '+cast(@deptid as varchar(20))+' not in (378,841) and YFMC in(''iv drip'',''H'',''iv'',''im'',''皮下注射'',''iv pump'')  then 0 else b.sl*b.js*c.dwbl/b.ydwbl end) ypsl,
    f.hwmc as hwmc,a.byscf        
