...
namespace ts_ybznsh_interface
{
    ...
    public class BmiAuditClass
    {
        ...
        public string f_constructxml1(DataTable inbill, DataTable inbilldetail)
        {
            ...
            string yblx = inbill.Rows[0]["YBLX"].ToString();
            string ybzlx = inbill.Rows[0]["XZLX"].ToString();
            if (yblx.Equals("1"))
            {
                inAGENCIES_ID = "1";//2省直医保  属于公费   //inbill.Rows[0]["inAGENCIES_ID"].ToString();
            }
            if (yblx.Equals("3") && ybzlx.Equals("55"))
            {
                inAGENCIES_ID = "2";//2省直医保  属于公费   //inbill.Rows[0]["inAGENCIES_ID"].ToString();
            }
            ...
            if (l_s.Equals("1"))
            {
                for (int ll_row = 0; ll_row < ii_billdetailrowcount; ++ll_row)
                {
                    ...
                    else
                    {
                        ...
                        {
                            //省医保 “AKC190”+明细id
                            IDb = ID + inbilldetail.Rows[ll_row]["ID"].ToString();
                        }
                        else
                        {
                            IDb = inbilldetail.Rows[ll_row]["ID"].ToString();
        ...
        public string ClaimAudit4Hospital_N(DataTable dt_bill, DataTable dt_billdetail)
        {
            ...
            {
                throw new Exception(" f_constructxml1 出错 " + ex.Message + "\r\r" + ex.StackTrace);
            }
            ...
            if (l_s.Equals("1"))
            {
                ...
                {
                    throw new Exception(" AuditClaimInfo_N 出错 " + ex.Message);
                }
        ...
        public string ClaimAudit4Hospital_Y(DataTable dt_bill, DataTable dt_billdetail)
        {
            ...
            {
                //throw new Exception(" f_constructxml1 出错 " + ex.Message);
                throw new Exception(" f_constructxml1 出错 " + ex.Message + "\r\r" + ex.StackTrace);
            }
            ...
            if (l_s.Equals("1"))
            {
                ...
                {
                    throw new Exception(" AuditClaimInfo_Y 出错 " + ex.Message);
                }
                //l_s = AuditClaimInfo_N("");
        ...
        public string ClaimAudit4Hospital_S(DataTable dt_bill, DataTable dt_billdetail)
        {
            ...
            if (l_s.Equals("1"))
            {
                ...
                {
                    throw new Exception(" AuditClaimInfo_S 出错 " + ex.Message);
                }
                //l_s = AuditClaimInfo_N("");
                //l_s = AuditClaimInfo_Y("");//wait jchl  AuditClaimInfo_Y
                //l_s = AuditClaimInfo_S();
