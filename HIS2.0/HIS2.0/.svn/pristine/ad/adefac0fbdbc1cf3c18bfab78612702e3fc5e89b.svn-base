USE [Trasen]
GO
/****** Object:  StoredProcedure [dbo].[sp_yp_tj_jxchzb_kshb_hh]    Script Date: 10/15/2015 19:10:08 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[sp_yp_tj_jxchzb_kshb_hh]     
(    
 @yk int,    
 @jhjetj int,    
 @year int,    
...
exec sp_yp_tj_jxchzb_kshb_hh @yk=0,@jhjetj=0,@year=2014,@month=7,@err_code=@err_code output,@err_text=@err_text output    
...
select cast(YPLX as varchar(50))+'000'+CAST(left('00000',5-LEN(d.fzxh)) as varchar(50))+cast(d.fzxh as varchar(30)),
...
inner join YP_YJKS d on a.DEPTID=d.DEPTID
 group by d.fzxh ,yplx,a.ywlx,WLDW,qc,qm   
if @month=0    
begin    
...
---准备药剂科室    
create table #yjks(ksmc varchar(50),deptid bigint,pxxh int)    
if @yk=1     
 insert into #yjks    
 select  rtrim(a.fzmc)+dbo.fun_yp_yplx(c.yplx) as xmmc ,cast(c.YPLX as varchar(50))+'000'+CAST(left('00000',5-LEN(a.fzxh)) as varchar(50))+cast(a.fzxh as varchar(30)),a.fzxh from yp_yjks a inner join jc_dept_property b on a.DEPTID=b.DEPT_ID and (b.NAME like '%后湖%' or b.NAME like '%静脉%') and kslx='药库'
 inner join YP_GLLX c on a.DEPTID=c.DEPTID  where rtrim(a.fzmc) is not null and dbo.fun_yp_yplx(c.yplx) is not null group by  a.fzmc,c.YPLX,a.fzxh   order by a.fzxh  
else    
 insert into #yjks    
 select  rtrim(a.fzmc)+dbo.fun_yp_yplx(c.yplx) as xmmc ,cast(c.YPLX as varchar(50))+'000'+CAST(left('00000',5-LEN(a.fzxh)) as varchar(50))+cast(a.fzxh as varchar(30)),a.fzxh from yp_yjks a inner join jc_dept_property b on a.DEPTID=b.DEPT_ID and (b.NAME like '%后湖%' or b.NAME like '%静脉%') and kslx='药房'
 inner join YP_GLLX c on a.DEPTID=c.DEPTID where rtrim(a.fzmc) is not null and dbo.fun_yp_yplx(c.yplx) is not null   group by  a.fzmc,c.YPLX,a.fzxh   order by a.fzxh      
