...
namespace ts_yf_cx
{
    ...
    public class Frmqykccx : System.Windows.Forms.Form
    {
        ...
        private void myDataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
        {
            ...
            string ssql = "select dbo.fun_getdeptname(deptid) 科室,cast(sum(kcl/dwbl) as decimal(15,3)) 数量,dbo.fun_yp_ypdw(ypdw) 单位,cast(bdelete_kc as smallint) 禁用 from vi_yp_kcmx a where cjid=" + Convert.ToInt32(tb.Rows[nrow]["cjid"]) + "";
            if (Convert.ToInt32(cmbjgbm.SelectedValue) > 0)
                ssql = ssql + " and jgbm=" + Convert.ToInt64(cmbjgbm.SelectedValue) + " ";
        ...
        private void butexcel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string bz = "";
                bz = bz + "";
                string swhere = " 库存金额 " + tb.Compute("sum(零售金额)","").ToString();
                // 设置条件
