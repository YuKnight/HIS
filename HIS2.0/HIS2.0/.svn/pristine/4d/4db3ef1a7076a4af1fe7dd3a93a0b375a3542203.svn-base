IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[sp_yp_tj_jxchzb_yplx_ny]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[sp_yp_tj_jxchzb_yplx_ny]
GO
CREATE PROCEDURE [dbo].[sp_yp_tj_jxchzb_yplx_ny]     
(    
 @yk int,    
 @jhjetj int,    
 @year int,    
...
exec sp_yp_tj_jxchzb_yplx_ny @yk=0,@jhjetj=0,@year=2014,@month=7,@err_code=@err_code output,@err_text=@err_text output    
...
--业务发生    
create table #temp (deptid bigint,ywlx varchar(30),wldw int,jhje decimal(15,2),lsje decimal(15,2),qc int ,qm int ,wldwlx int ,tjlx varchar(30),tjlxpxfs int,ywlxpxfs int)    
insert into #temp(deptid,ywlx,wldw,jhje,lsje,qc,qm)    
select cast(YPLX as varchar(50))+'000'+CAST(left('00000',5-LEN(a.deptid)) as varchar(50))+cast(a.deptid as varchar(30)),
a.ywlx,A.WLDW,SUM(JHJE) JHJE,SUM(LSJE) LSJE,qc,qm from YP_TJ_YMJCMX a inner join #tempyjjl b on a.yjid=b.yjid   
inner join VI_YP_YPCD c on a.CJID=c.cjid  
 group by a.deptid,yplx,a.ywlx,WLDW,qc,qm    
if @month=0    
begin    
...
---准备药剂科室    
create table #yjks(ksmc varchar(100),deptid bigint,pxxh int)    
if @yk=1     
 insert into #yjks    
 select  rtrim(KSMC)+rtrim(dbo.fun_yp_yplx(c.yplx)) as xmmc ,cast(c.YPLX as varchar(50))+'000'+CAST(left('00000',5-LEN(a.deptid)) as varchar(50))+cast(a.deptid as varchar(30)),a.pxxh from yp_yjks a inner join jc_dept_property b on a.DEPTID=b.DEPT_ID and not (b.NAME like '%后湖%' or b.NAME like '%静脉%' or b.NAME like '%谌家矶%' or b.NAME like '%花桥%') and kslx='药库'
 inner join YP_GLLX c on a.DEPTID=c.DEPTID  group by  KSMC,c.YPLX,a.pxxh,a.deptid order by a.pxxh  
else    
 insert into #yjks    
 select  rtrim(KSMC)+rtrim(dbo.fun_yp_yplx(c.yplx)) as xmmc ,cast(c.YPLX as varchar(50))+'000'+CAST(left('00000',5-LEN(a.deptid)) as varchar(50))+cast(a.deptid as varchar(30)),a.pxxh from yp_yjks a inner join jc_dept_property b on a.DEPTID=b.DEPT_ID and not (b.NAME like '%后湖%' or b.NAME like '%静脉%' or b.NAME like '%谌家矶%' or b.NAME like '%花桥%') and kslx='药房'
 inner join YP_GLLX c on a.DEPTID=c.DEPTID  group by  KSMC,c.YPLX,a.pxxh,a.deptid order by a.pxxh    
  
 
---------------------------------------------期初与结存----------------------------------------------------------------_    
create table #tempX (xmmc varchar(100),deptid bigint,jhje decimal(15,2),lsje decimal(15,2),pxfs varchar(30))    
...
(case when ywlx='000' then '0' else '999' end) from #temp a where ywlx in('000')  group by ywlx,deptid    
if @@ROWCOUNT=0    
begin    
 insert into #tempX(xmmc,deptid,jhje,lsje,pxfs)     
...
insert into #tempX(xmmc,deptid,jhje,lsje,pxfs)     
select (case when ywlx='000' then '一、期初结存' else '五、期未结存' end) xmmc ,deptid,SUM(jhje),SUM(lsje),    
(case when ywlx='000' then '0' else '999' end) from #temp a where ywlx in('999')  group by ywlx,deptid    
if @@ROWCOUNT=0    
begin    
 insert into #tempX(xmmc,deptid,jhje,lsje,pxfs)     
...
insert into #tempX(xmmc,deptid,jhje,lsje,pxfs)     
select  '三、本期减少'  xmmc ,deptid,
SUM((case when ywfx<>'-' then jhje*(-1) else jhje end)),
SUM((case when ywfx<>'-' then lsje*(-1) else lsje end)),
'3'    
...
if @jhjetj=0 set @je='lsje'    
 ...
 --select @sql = @sql + 'sum(case name when '''+rtrim(ksmc)+'''      
 --then '+@je+' else null end) as '''+ rtrim(ksmc) +''','      
 --from (select   ksmc from #yjks  ) as a      
 --select @sql = left(@sql,len(@sql)-1)       
 --select @sql = @sql + ' into #result from #tempX a  '+      
 --' inner join  jc_dept_property  b  '+      
 --'ON a.deptid=b.dept_id  group by a.xmmc,pxfs order by pxfs'      
 --exec(@sql + ';     
 --select * from #result a  ;')      
      
 set @sql = 'select xmmc 项目,sum('+@je+') as 合计,'      
 select @sql = @sql + 'sum(case rtrim(b.ksmc) when '''+rtrim(ksmc)+'''      
 then '+@je+' else null end) as '''+ rtrim(ksmc) +''','      
 from (select   ksmc from #yjks  ) as a      
 select @sql = left(@sql,len(@sql)-1)       
 select @sql = @sql + ' into #result from #tempX a  '+      
