...
namespace ts_mz_class
{
    ...
    public class FunAddComboBox
    {
        ...
        public static void AddKlx(bool all, int klx, System.Windows.Forms.ComboBox cmb, RelationalDatabase _DataBase)
        {
            string ssql = "";
            if (klx == 0)
                ssql = "select KLX,KLXMC from JC_KLX where bqybz=1 order by klx";
            else
        ...
        /// <param name="klx">卡类型</param>
        /// <param name="cmb">下拉列表控件</param>
        public static void AddKlx(bool all, int klx, int kcd, System.Windows.Forms.ComboBox cmb, RelationalDatabase _DataBase)
        {
            string ssql = "";
            if (klx == 0)
                ssql = string.Format("select KLX,KLXMC from JC_KLX where bqybz=1 and kcd<>{0} order by XH", kcd);
            else
                ssql = "select KLX,KLXMC from JC_KLX where  bqybz=1  and klx=" + klx;
            DataTable tb = _DataBase.GetDataTable(ssql);
        ...
        /// <param name="ghjb">挂号级别</param>
        /// <param name="cmb">下拉列表控件</param>
        public static void AddGhjb(bool all, System.Windows.Forms.ComboBox cmb, RelationalDatabase _DataBase)
        {
            string ssql = "";

            ssql = "SELECT TYPE_ID,idqz + '-'+ type_name AS TYPENAME from jc_doctor_type where zcjb in (1,2)";
        ...
        /// <param name="cmb"></param>
        /// <param name="_DataBase"></param>
        public static void AddYhfa(int klx, Guid kdjid, int brlx, int yblx, int htdwlx, string funname, string ksdm, System.Windows.Forms.ComboBox cmb, RelationalDatabase _DataBase)
        {
            ...
            parameters[6].Value = ksdm;

            DataTable tb = _DataBase.GetDataTable("SP_MZSF_GETYHLXBYKS", parameters, 30);
            cmb.ValueMember = "id";
            cmb.DisplayMember = "yhmc";
            cmb.DataSource = tb;
        ...
        ///  数据字典
        /// </summary>
        /// <param name="DDFlId"></param>
        /// <param name="all"></param>
        /// <param name="isdefault"></param>
        /// <param name="selVal"></param>
        /// <param name="cmb"></param>
        /// <param name="_DataBase"></param>
        public static void AddDic(string DDFlId, bool bolAll, bool bolNull, bool bolDefault, string selVal, System.Windows.Forms.ComboBox cmb, RelationalDatabase _DataBase)
        {
            if (String.IsNullOrEmpty(DDFlId))
                return;
            string ssql = "select a.Code,a.Name,a.IsDefault from JC_DataDictionary a,JC_DataDictionary_class b where a.FlId=b.Id and IsDelete !=1 and a.FlId='" + DDFlId + "'";
            DataTable tb = _DataBase.GetDataTable(ssql);
            if (bolAll == true)
            {
                DataRow row = tb.NewRow();
            ...
            if (!String.IsNullOrEmpty(selVal))
            {
                ...
                {
                    if (tb.Rows[i]["code"].ToString() == selVal)
                    {
                        cmb.SelectedIndex = i;
                        break;
                    }
            ...
            else if (bolDefault)
            {
                for (int i = 0; i < tb.Rows.Count; i++)
                {
                    if (tb.Rows[i]["IsDefault"].ToString() == "1")
                    {
                        cmb.SelectedIndex = i;
                        break;
                    }
