...
namespace ts_mz_class
{
    public class Fun
    {
        ...
        public static string GetNewDnlsh(int czyh, long jgbm, RelationalDatabase _DataBase)
        {
            try
            {
                string dnlsh = "";
                ...
                parameters[0].Text = "@CZY";
                parameters[0].Value = czyh;

                parameters[1].Text = "@JGBM";
                parameters[1].Value = jgbm;// TrasenFrame.Forms.FrmMdiMain.Jgbm;

                parameters[2].Text = "@DNLSH";
                parameters[2].ParaDirection = ParameterDirection.Output;
                parameters[2].ParaSize = 100;

                //ps.Add(new ParameterEntity("@CZY", czyh));
                //ps.Add(new ParameterEntity("@JGBM", jgbm));
                //ps.Add(new ParameterEntity("@DNLSH", dnlsh, 100, DataTypeEnum.String, ParameterDirectionEnum.Output));
                //dal.GetExecuteNonQuery("SP_MZ_NEW_DNLSH_CZY", CommandTypeEnum.StoredProcedure, ps);
                _DataBase.DoCommand("SP_MZ_NEW_DNLSH_CZY", parameters, 30);
                 dnlsh = Convert.ToString(parameters[2].Value);
                return dnlsh;
            }
            catch
            {
        ...
        public static DataTable GetGhys(int jb, RelationalDatabase _DataBase)
        {
            ...
            if (jb > 0)
                ssql = ssql + " and ys_typeid in (select zcjb from  JC_DOCTOR_TYPE where type_id =" + jb + ")";
            return _DataBase.GetDataTable(ssql);
        }
        /// <summary>
        /// 获取挂号医生 
        ...
        /// <param name="jb">级别</param>
        /// <param name="flag">1只根据级别获取，2根据科室和级别获取</param>
        /// <param name="_DataBase">相关数据库连接对象</param>
        /// <returns>挂号医生列表</returns>
        public static DataTable GetGhys(int ksdm, int jb, int flag, RelationalDatabase _DataBase)
        {
            try
            {
                ParameterEx[] parameters = new ParameterEx[3];
                ...
                parameters[1].Text = "@JB";
                parameters[1].Value = jb;

                parameters[2].Text = "@FLAG";
                parameters[2].Value = flag;

                DataTable tb = _DataBase.GetDataTable("SP_MZGH_GETGHYSPBXX", parameters, 30);
                return tb;
            }
            catch (System.Exception err)
            {
        ...
        {
            return GetXmYp_YZ(all, xmly, zyyf, execdept, deptid, pym, wbm, itemname, jgbm, sjkz, _DataBase, funname, 0);
        }
        public static DataTable GetXmYp_YZ(int all, int xmly, int zyyf, int execdept, int deptid, string pym, string wbm, string itemname, long jgbm, int sjkz, RelationalDatabase _DataBase, string funname, int ylfl)
        {
            try
            {
                ...
                parameters[11].Text = "@ylfl";
                parameters[11].Value = ylfl;
        ...
        public static string GetJshm(string kshm, int zs)
        {
            ...
            string jshm = Convert.ToString(Convert.ToDecimal(kshm) + zs - 1);
            if (jshm.Length != kshm.Length)
                jshm = jshm.PadLeft(kshm.Length, '0');
            return jshm;
        }
        ...
        public static string M_ConfirmBrxx(Guid brxxid, RelationalDatabase _DataBase)
        {
            try
            {
                SystemCfg cfg1173 = new SystemCfg(1173);

                SystemCfg cfg1174 = new SystemCfg(1174);
                if (cfg1173 == null || cfg1173.Config != "1")
                {
                    return "";
                }
                if (cfg1174 == null || String.IsNullOrEmpty(cfg1174.Config))
                {
                    return "";
                }
                ...
                string str = string.Format(@"select brxxid,brxm,XB,CSRQ,hyzk,gj,mz,zy,csdz,jtdz,jtyb,jtdh,jtlxr,
brlxfs,dzyj,gzdw,gzdwdz,gzdwyb,gzdwdh,gzdwlxr,sfzh,brlx,yblx,cbkh,
djsj,djy,scxgsj,scxgczy,djly,bscbz,pym,wbm,idcardserno 
from YY_BRXX where BRXXID='{0}' ", brxxid);
                DataTable dt = _DataBase.GetDataTable(str);
                if (dt == null || dt.Rows.Count == 0)
                {
                    return "";
                }
                string[] arr_colname = cfg1174.Config.Split(',');
                StringBuilder sbr_warn = new StringBuilder();
                foreach (string item in arr_colname)
                {
                    if (String.IsNullOrEmpty(dt.Rows[0][item].ToString()))
                    {
                        switch (item)
                        {
                            case "brxm":
                            ...
                            case "XB":
                                ...
                                break;
                            case "CSRQ":
                                sbr_warn.Append("出生日期，");
                                break;
                            case "hyzk":
                                sbr_warn.Append("婚姻状况，");
                                break;
                            case "gj":
                                sbr_warn.Append("国籍，");
                                break;
                            ...
                            case "zy":
                                ...
                                break;
                            case "csdz":
                                sbr_warn.Append("出生地址，");
                                break;
                            case "jtdz":
                                sbr_warn.Append("家庭地址，");
                                break;
                            case "sfzh":
                                sbr_warn.Append("身份证号，");
                                break;
                            case "brlxfs":
                                sbr_warn.Append("本人联系方式，");
                                break;
                            case "jtyb":
                                sbr_warn.Append("家庭地址邮编，");
                                break;
                            case "jtdh":
                                sbr_warn.Append("家庭电话，");
                                break;
                            case "jtlxr":
                                sbr_warn.Append("家庭联系人，");
                                break;
                            case "dzyj":
                                sbr_warn.Append("电子邮件，");
                                break;
                            case "gzdw":
                                sbr_warn.Append("工作单位，");
                                break;
                            case "gzdwdz":
                                sbr_warn.Append("工作单位地址，");
                                break;
                            case "gzdwyb":
                                sbr_warn.Append("工作单位邮编，");
                                break;
                            case "gzdwdh":
                                sbr_warn.Append("工作单位电话，");
                                break;
                            case "gzdwlxr":
                                sbr_warn.Append("工作单位联系人，");
                                break;
                            case "brlx":
                                sbr_warn.Append("病人类型，");
                                break;
                            case "yblx":
                                sbr_warn.Append("医保类型，");
                                break;
                            case "cbkh":
                                sbr_warn.Append("参保卡号，");
                                break;
                            default:
                                break;
                        }
                ...
                if (sbr_warn.Length > 0)
                {
                    sbr_warn.Insert(0, "请完善：");
                    return sbr_warn.ToString().Substring(0, sbr_warn.Length - 1);
                }
                return "";
            }
            catch (Exception ex)
