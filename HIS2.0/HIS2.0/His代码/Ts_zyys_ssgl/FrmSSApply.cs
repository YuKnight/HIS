...
namespace Ts_zyys_ssgl
{
    ...
    public class FrmSSApply : System.Windows.Forms.Form
    {
        ...
        private void InitializeComponent()
        {
            ...
            this.btSH.Location = new System.Drawing.Point(496, 421);
            this.btSH.Name = "btSH";
...
//                }
            ...
            #region add  by 岳成成 2014-08-11
            int ysss = 0;
            try
            {
                ysss= Convert.ToInt32(this.txtysss.Tag.ToString().Trim());
                string sql = "select ORDER_ID from JC_HOITEMDICTION where ORDER_TYPE=6 and ORDER_NAME ='" + this.txtysss.Text.Trim() + "'";
                DataTable dtORDER_ID = new DataTable();
                dtORDER_ID = InstanceForm._database.GetDataTable(sql);
                sqlstring = "select * from JC_HOITEMDICTION where ORDER_TYPE=6 and ORDER_ID='" + dtORDER_ID.Rows[0]["ORDER_ID"]+"'";
            }
            catch 
            {
                ysss = 0;
            ...
            else
            {
                ...
                dt = InstanceForm._database.GetDataTable(sqlstring);
                if (dt.Rows.Count == 0)
                {
                    MessageBox.Show("拟施手术必须填写！", "提示");
                    this.txtysss.Focus();
