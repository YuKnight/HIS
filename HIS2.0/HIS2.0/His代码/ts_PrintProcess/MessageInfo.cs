...
namespace ts_PrintProcess
{
    ...
    public class MessageInfo
    {
        ...
        public MessageInfo()
        {
            this.senderId = FrmMdiMain.CurrentUser.EmployeeId;
            this.senderName = FrmMdiMain.CurrentUser.Name;
            ...
            this.sendTime = TrasenClasses.GeneralClasses.DateManager.ServerDateTimeByDBType(FrmMdiMain.Database);
        }
        /// <summary>
        /// 根据ID构造消息对象
        /// </summary>
        ...
        public static List<MessageInfo> GetUnDealMessageList(RelationalDatabase database)
        {
            ...
            //    FrmMdiMain.CurrentUser.EmployeeId , FrmMdiMain.CurrentSystem.SystemId , FrmMdiMain.CurrentDept.DeptId );

            ////发给自己所在病区或科室的消息
            //string str2 = string.Format( "select * from Pub_Message_Record Where DealStatus=0 and ReciveStaff = 0 and ( (ReciveWard<>'' and ReciveWard = (select ward_id from JC_WARD where dept_id = {0})) or (ReciveDept> 0 and ReciveDept = {0} ))" , FrmMdiMain.CurrentDept.DeptId );
            ////发给自己当前登录的系统的消息
            //string str3 = string.Format( "select * from Pub_Message_Record where DealStatus=0 and ReciveStaff = 0 and ( ReciveSystem> 0 and ReciveSystem In (select Module_id from pub_system where System_Id ={0} )  )" , FrmMdiMain.CurrentSystem.SystemId );
            ...
            return GetUnDealMessageList(FrmMdiMain.CurrentSystem.SystemId, FrmMdiMain.CurrentDept.DeptId,
                FrmMdiMain.CurrentUser.EmployeeId, database);
        }
