...
namespace ts_mz_class
{
    public class jc_mb
    {
        ...
        public static void SaveMb( jc_mb mb, out Guid NewMbid , out int err_code , out string err_text , RelationalDatabase _DataBase )
        {
            try
            {
                err_code = 0;
                err_text = "";
                using ( IDbCommand cmd = _DataBase.GetCommand() )
                {
                    if ( _DataBase.IsInTransaction )
                        cmd.Transaction = _DataBase.GetTransaction();
                    cmd.CommandType = CommandType.Text;
                    if ( mb.MBXH == Guid.Empty )
                    {
                        NewMbid = Guid.NewGuid();
                        cmd.CommandText = "insert into jc_cfmb(mbxh,jgbm,mbmc,pym,wbm,bz,mbjb,ksdm,ysdm,djsj,djy,FID) values(@mbid,@jgbm,@mbmc,@pym,@wbm,@bz,@mbjb,@ksdm,@ysdm,@djsj,@djy,@fid)  ";
                        cmd.Parameters.Add( Fun.NewCommandParameter( cmd , "@mbid" , NewMbid ) );
                    }
                    else
                    {
                        NewMbid = mb.MBXH;
                        cmd.CommandText = "update jc_cfmb set mbmc=@mbmc,pym=@pym,wbm=@wbm,mbjb=@mbjb,ksdm=@ksdm,ysdm=@ysdm where mbxh=@mbid  ";
                        cmd.Parameters.Add( Fun.NewCommandParameter( cmd , "@mbid" , mb.MBXH ) );
                    }
                    #region parameters
                    cmd.Parameters.Add( Fun.NewCommandParameter( cmd , "@jgbm" , mb.JGBM ) );
                    cmd.Parameters.Add( Fun.NewCommandParameter( cmd , "@mbmc" , mb.MBMC ) );
                    cmd.Parameters.Add( Fun.NewCommandParameter( cmd , "@pym" , mb.PYM ) );
                    cmd.Parameters.Add( Fun.NewCommandParameter( cmd , "@wbm" , mb.WBM ) );
                    cmd.Parameters.Add( Fun.NewCommandParameter( cmd , "@bz" , mb.BZ ) );
                    cmd.Parameters.Add( Fun.NewCommandParameter( cmd , "@mbjb" , mb.MBJB ) );
                    cmd.Parameters.Add( Fun.NewCommandParameter( cmd , "@ksdm" , mb.KSDM ) );
                    cmd.Parameters.Add( Fun.NewCommandParameter( cmd , "@ysdm" , mb.YSDM ) );
                    cmd.Parameters.Add( Fun.NewCommandParameter( cmd , "@zxks" , mb.ZXKS  ) );
                    cmd.Parameters.Add( Fun.NewCommandParameter( cmd , "@djsj" , mb.DJSJ ) );
                    cmd.Parameters.Add( Fun.NewCommandParameter( cmd , "@djy" , mb.DJY ) );
                    cmd.Parameters.Add( Fun.NewCommandParameter( cmd , "@fid" , mb.FID ) );
                    #endregion
                    ...
                    if ( ret == 1 )
                    {
                        ...
                        string _bz = "";
                        if ( mb.MBXH == Guid.Empty )
                            _bz = Fun.SeekEmpName( mb.DJY , _DataBase ) + " 新增模板 " + mb.MBMC;
                        else
                            _bz = Fun.SeekEmpName( mb.DJY , _DataBase ) + " 修改模板 " + mb.MBMC;

                        ts.Save_log( ts_HospData_Share.czlx.jc_处方模板 , _bz , "jc_cfmb" , "mbxh" , NewMbid.ToString() , TrasenFrame.Forms.FrmMdiMain.Jgbm , 0 , -999 , "" , out log_djid , _DataBase );
                    }
                    else
                        throw new Exception( "保存模板头失败，没有插入或更新到记录" );
                }
...
//                 ) A ORDER BY cfxh,排序序号 ";
                ...
                (
                    select '' 序号,cast(0 as smallint) 选择 ,yppm+'  '+ypgg as 医嘱内容,cast(cast(jl as float) as varchar(30)) 剂量,  
                        jldw 剂量单位,rtrim(dbo.Fun_getFreqName(pc)) 频次,dbo.fun_getUsageName(yf) 用法,ts 天数,b.zt 嘱托,    
                        dbo.FUN_YP_DW_LSJ(b.DWLX,b.SL,1,1,b.TS,b.CJID,b.ZXKS,0) 单价,(case JS when 0 then '1' else cast(js as varchar(10) ) END)  剂数, 
                        sl 总量,DW 单位, dbo.fun_getdeptname(zxks) 执行科室, 
                        B.cfxh as cfxh,jldwid 剂量单位id,dwlx, cast(pc as varchar(20)) 频次id,cast(yf as varchar(20)) 用法ID, xmid 项目ID,bzby 自备药,    
                        fzxh 处方分组序号,pxxh 排序序号,zxks 执行科室id,xmly 项目来源 ,b.cjid,c.STATITEM_CODE 统计大项目,a.bz     
                    from jc_cfmb a  
                        inner join jc_cfmb_mx b on a.mbxh=b.mbxh   
                        inner join yp_ypggd c on b.xmid=c.ggid   
                        left join yp_ypcjd d on b.cjid=d.cjid
                    ...
                    select '' 序号,cast(0 as smallint) 选择, order_name as 医嘱内容,cast(cast(jl as float) as varchar(30)) 剂量,  
                        ...
                        B.cfxh as cfxh,jldwid 剂量单位id,dwlx,cast(pc as varchar(20)) 频次id,cast(yf as varchar(20)) 用法ID,xmid 项目ID,bzby 自备药, fzxh 处方分组序号,pxxh 排序序号,       
                        (case when dbo.Fun_GetOrderExecDept(xmid," + jgbm + @",b.zxks)=0 then b.zxks else dbo.Fun_GetOrderExecDept(xmid," + jgbm + @",b.zxks) end ) as 执行科室id,
                        xmly 项目来源 , b.cjid,'' 统计大项目 ,a.bz   
                    from jc_cfmb a  
                        inner join jc_cfmb_mx b on a.mbxh=b.mbxh  
                        inner join jc_hoitemdiction c on b.xmid=c.order_id  
                        left join (  
                    ...
                    union all 
                    select '' 序号,cast(0 as smallint) 选择, b.xmmc as 医嘱内容,cast(cast(jl as float) as varchar(30)) 剂量,  
                        jldw 剂量单位,rtrim(dbo.Fun_getFreqName(pc)) 频次,dbo.fun_getUsageName(yf) 用法,ts 天数,zt 嘱托, price 单价,cast(js as varchar(10)) 剂数,   
                        sl 总量,DW 单位,dbo.fun_getdeptname(case when dbo.Fun_GetOrderExecDept(xmid," + jgbm + @",b.zxks)=0 then b.zxks else dbo.Fun_GetOrderExecDept(xmid," + jgbm + @",b.zxks) end) as 执行科室, 
                        B.cfxh as cfxh,jldwid 剂量单位id,dwlx,cast(pc as varchar(20)) 频次id,cast(yf as varchar(20)) 用法ID,xmid 项目ID,bzby 自备药,fzxh 处方分组序号,pxxh 排序序号,    
                        (case when dbo.Fun_GetOrderExecDept(xmid," + jgbm + @",b.zxks)=0 then b.zxks else dbo.Fun_GetOrderExecDept(xmid," + jgbm + @",b.zxks) end ) as 执行科室id,
                        xmly 项目来源 ,b.cjid,'' 统计大项目,a.bz   
                    from jc_cfmb a  
                        inner join jc_cfmb_mx b on a.mbxh=b.mbxh  and b.XMID=-1  and b.zxks not in(select ZXKSID from JC_KDKSXZ where KDKSID=" + deptid + @")  
                        left join jc_hoitemdiction c on b.xmid=c.order_id  
                        left join (  
                            select jgbm,hoitem_id,price from jc_hoi_hdi a inner join jc_hsitemprice b on a.hditem_id=b.hsitemid and tc_flag=0 union   
                            all  
                    ...
                    union all
                    select '' 序号,cast(0 as smallint) 选择, b.xmmc as 医嘱内容,cast(cast(jl as float) as varchar(30)) 剂量,  
                        jldw 剂量单位,rtrim(dbo.Fun_getFreqName(pc)) 频次,dbo.fun_getUsageName(yf) 用法,ts 天数,zt 嘱托,price 单价,cast(js as varchar(10)) 剂数, sl 总量,DW 单位,  
                        dbo.fun_getdeptname(case when dbo.Fun_GetOrderExecDept(xmid," + jgbm + @",b.zxks)=0 then b.zxks else dbo.Fun_GetOrderExecDept(xmid," + jgbm + @",b.zxks) end) as 执行科室, 
                        B.cfxh as cfxh,jldwid 剂量单位id,dwlx, cast(pc as varchar(20)) 频次id,cast(yf as varchar(20)) 用法ID, xmid 项目ID,bzby 自备药,fzxh 处方分组序号,pxxh 排序序号,         
                        (case when dbo.Fun_GetOrderExecDept(xmid," + jgbm + @",b.zxks)=0 then b.zxks else dbo.Fun_GetOrderExecDept(xmid," + jgbm + @",b.zxks) end ) as 执行科室id,
                        xmly 项目来源 ,  b.cjid ,'' 统计大项目 ,a.bz   
        ...
        public static DataTable GetMzsfMbmx(Guid mbid,long jgbm, RelationalDatabase _DataBase)
        {
            ...
            try
            {
                ...
                from jc_cfmb a  
                inner join jc_cfmb_mx b on a.mbxh=b.mbxh  
                inner /*left*/ join YP_YPCJD c on b.XMID=c.CJID
                where a.mbxh='" + mbid + @"' and xmly = 1 
                union all
                ...
                where a.mbxh='" + mbid + @"'  and b.xmly=2 and b.tc_flag = 0
               union all
                select '' 序号,cast(0 as smallint) 选择, c.ITEM_NAME as 医嘱内容,cast(cast(jl as float) as varchar(30)) 剂量,  
                jldw 剂量单位,rtrim(dbo.Fun_getFreqName(pc)) 频次,dbo.fun_getUsageName(yf) 用法,ts 天数,zt 嘱托,sl 总量,DW 单位,  
                dbo.fun_getdeptname(case when dbo.Fun_GetOrderExecDept(xmid," + jgbm+",b.zxks)=0 then b.zxks else dbo.Fun_GetOrderExecDept(xmid,"+jgbm+@",b.zxks) end) as 执行科室,  
                ...
                b.cjid,'' 统计大项目,b.PXXH,tc_flag,a.bz
                from jc_cfmb a  
                inner join jc_cfmb_mx b on a.mbxh=b.mbxh and b.xmly=2 and b.tc_flag<>0
