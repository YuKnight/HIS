    ...
    public partial class Frmyyjl : Form
    {
        ...
        private void butcx_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    //Modify by zp 2014-09-26
                    DataTable tb = Mz_YYgh.GetYYghInfo( "" , txtsfzh.Text.Trim() , kh , txtyzm.Text , Mz_YYgh.YYgh_Status.未作废未取号记录 , "" , "" , klx , 0 , 0 , Mz_YYgh.YYgh_Sort.所有预约方式 , out err_code , out err_text , TrasenFrame.Forms.FrmMdiMain.Database );
                    dataGridView1.DataSource = tb;
        ...
        private void butok_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    int nrow = dataGridView1.CurrentCell.RowIndex;
                    //预约卡号为空：判断刷卡病人与预约病人是否同名；不为空，直接挂号预约卡号。
                    if (tb.Rows[nrow]["卡号"].ToString().Trim() == "")
                    {
                        if (brxm.Trim() != "" && brxm.Trim() != tb.Rows[nrow]["姓名"].ToString().Trim())
                        {
                            if (MessageBox.Show(string.Format("预约姓名【{0}】与当前病人姓名【{1}】不一致,是否继续为【{1}】挂号？", tb.Rows[nrow]["姓名"].ToString().Trim(), brxm.Trim()), "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                                return;
                        }
                    }
                    ...
                    ReturnNewKH = Convertor.IsNull(this.dataGridView1.SelectedRows[0].Cells["卡号"].Value, in_kh.Trim());
                    Bok = true;
                    this.Close();
                }
                else
