...
namespace ts_yp_xtwh
{
	...
	/// Form1 的摘要说明。
	/// </summary>
	public class Frmywy : System.Windows.Forms.Form
	{
		...
		public Frmywy(MenuTag menuTag,string chineseName,Form mdiParent)
		{
			//
			// Windows 窗体设计器支持所必需的
			//
		...
		private void InitializeComponent()
		{
            ...
            this.dataGridTextBoxColumn6.HeaderText = "所属供货单位";
            ...
            this.dataGridTextBoxColumn7.HeaderText = "ghdwid";
            ...
            // butprint
            // 
            this.butprint.Location = new System.Drawing.Point(736, 21);
            ...
            // butquit
            // 
            this.butquit.Location = new System.Drawing.Point(875, 21);
            ...
            // butdel
            // 
            this.butdel.Location = new System.Drawing.Point(597, 21);
            ...
            // butsave
            // 
            this.butsave.Location = new System.Drawing.Point(459, 21);
		...
		private void AddData(string ss)
		{
			...
			if (ss.Trim()=="")
			{
				ssql="select 0 序号,ywymc 姓名,pym 拼音码,wbm 五笔码,lxdh 联系电话,dbo.fun_yp_ghdw(ghdwid) 所属供货单位,ghdwid,id from yp_ywy";
			}
			else
			{
				ssql="select 0 序号,ywymc 姓名,pym 拼音码,wbm 五笔码,lxdh 联系电话,dbo.fun_yp_ghdw(ghdwid) 所属供货单位,ghdwid,id from yp_ywy where pym like '"+txtdm.Text.Trim()+"%' or wbm like '"+txtdm.Text.Trim()+"%' or ywymc like '%"+txtdm.Text.Trim()+"%' ";
			}
		...
		private void butdel_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				//看是否有过使用记录
				string ssql="select * from yk_dj where jsr="+ID+" and (ywlx='001' or ywlx='002')";
				DataTable tab=InstanceForm.BDatabase.GetDataTable(ssql);
				if (tab.Rows.Count>0){MessageBox.Show("对不起，这个业务员在系统中已经使用了，不能删除");return;}

                InstanceForm.BDatabase.BeginTransaction();
                ssql = "delete from yp_ywy where id=" + ID + "";
                InstanceForm.BDatabase.DoCommand(ssql);

                //三院数据处理_____保存日志 
                if (ID > 0)
                {
                    string bz = "删除药品业务员 【" + tb.Rows[nrow]["姓名"].ToString() + "】 ";

                    ts.Save_log(ts_HospData_Share.czlx.yp_药品基础数据单表修改, bz, "yp_ywy", "id", ID.ToString(), InstanceForm._menuTag.Jgbm, 0, "", out log_djid, InstanceForm.BDatabase);
                }
		...
		private void butsave_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				string ssql="";
				string ywymc="";
				string pym="";string wbm="";string lxdh="";int ghdwid=0;
				
				for(int i=0;i<=tb.Rows.Count-1;i++)
				{

					int ID=Convert.ToInt32(Convertor.IsNull(tb.Rows[i]["id"],"0"));
					ywymc=tb.Rows[i]["姓名"].ToString();
					pym=tb.Rows[i]["拼音码"].ToString();
					wbm=tb.Rows[i]["五笔码"].ToString();
					lxdh=tb.Rows[i]["联系电话"].ToString();
					ghdwid=Convert.ToInt32(Convertor.IsNull(tb.Rows[i]["ghdwid"],"0"));

					if (ywymc.Trim()!="")
					{
						if (ID==0)
						{
							ssql="insert into yp_ywy(ywymc,pym,wbm,lxdh,ghdwid) values('"+ywymc+"','"+pym+"','"+wbm+"','"+lxdh+"',"+ghdwid+") ";
						}
						else
						{
							ssql="update yp_ywy set ywymc='"+ywymc+"',pym='"+pym+"',wbm='"+wbm+"',lxdh='"+lxdh+"',ghdwid="+ghdwid+" where id="+ID+" ";
						}
                        ...
                        if (ID == 0)
                        {
                            ...
                            ts.Save_log(ts_HospData_Share.czlx.yp_药品基础数据单表修改, InstanceForm.BCurrentUser.Name + "添加业务员 【" + ywymc + " 】", "yp_ywy", "id", newid.ToString(), InstanceForm._menuTag.Jgbm, 0, "", out log_djid, InstanceForm.BDatabase);
                            ss[i] = log_djid.ToString();
                        }
                        else
                        {
                            string _ssql = "select * from yp_ywy where id=" + ID + " and ywymc='" + ywymc + "' and pym='" + pym + "' and ghdwid=" + ghdwid + " ";
                            DataTable tb_x = InstanceForm.BDatabase.GetDataTable(_ssql);
                            if (tb_x.Rows.Count == 0)
                            {
                                ts.Save_log(ts_HospData_Share.czlx.yp_药品基础数据单表修改, InstanceForm.BCurrentUser.Name + "修改业务员 【" + ywymc + " 】", "yp_ywy", "id", ID.ToString(), InstanceForm._menuTag.Jgbm, 0, "", out log_djid, InstanceForm.BDatabase);
                                ss[i] = log_djid.ToString();
                            }
                            InstanceForm.BDatabase.DoCommand(ssql);
                        }
