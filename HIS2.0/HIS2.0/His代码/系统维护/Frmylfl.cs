...
//			node.Expand();
//			
//			TreeNode node1 = treeView2.Nodes.Add("药品分类目录");
//			node1.Tag="0";
//			node1.ImageIndex=0;
...
//						tb.Rows.Add(row);
		...
		private void butdel_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (tab.Rows.Count>0){MessageBox.Show("对不起，当前节点还有子节点，请先删除子节点","删除",MessageBoxButtons.OK,MessageBoxIcon.Error);return;}

				//看是否使用
				ssql="select * from yp_ypggd where ylfl="+ID+"";
				DataTable tab1=InstanceForm.BDatabase.GetDataTable(ssql);
				if (tab1.Rows.Count>0){MessageBox.Show("对不起，这个分类在药品词典中已经使用了，不能删除");return;}
							...
							if (hwbh.Trim()!=yhwbh.Trim())
							{
								...
								for (int j=0;j<=tab.Rows.Count-1;j++)
								{
                                    cj.Update_shh(Convert.ToInt32(tab.Rows[j]["ggid"]), out _errcode, out _errtext, InstanceForm.BDatabase);
									 if (_errcode!=0) throw new Exception("重新生成药品货位编号时发生错误");
								}
							}
						}
		...
		private void SelectTree1Node(string ssql,System.Windows.Forms.TreeViewEventArgs e)
		{
			...
			string ss="";
			if (ssql.Trim()=="")
				ss="select 0 序号,hWbh 货位编号,hwbh 原货位编号,flbh 编号,flmc  名称,pym 拼音码,wbm 五笔码,flms 描述,dbo.fun_yp_ylfl(fid) 所属上级编目,cast(bdelete as smallint) 禁用,fid,id,yjdbz from yp_ylfl where fid="+Convert.ToInt64(Convertor.IsNull(e.Node.Tag,"-10"))+" order by fid ";
			else
				ss="select 0 序号,hWbh 货位编号,hwbh 原货位编号,flbh 编号,flmc  名称,pym 拼音码,wbm 五笔码,flms 描述,dbo.fun_yp_ylfl(fid) 所属上级编目,cast(bdelete as smallint) 禁用,fid,id,yjdbz from yp_ylfl where wbm like '"+ssql.Trim()+"%' or pym like '"+ssql.Trim()+"%' order by fid ";
