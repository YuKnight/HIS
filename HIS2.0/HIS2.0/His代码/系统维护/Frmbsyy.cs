...
namespace ts_yp_xtwh
{
	...
	/// Form1 的摘要说明。
	/// </summary>
	public class Frmbsyy : System.Windows.Forms.Form
	{
		...
		public Frmbsyy(MenuTag menuTag,string chineseName,Form mdiParent)
		{
			//
			// Windows 窗体设计器支持所必需的
			//
		...
		private void InitializeComponent()
		{
            ...
            this.statusBar1.Size = new System.Drawing.Size(1158, 31);
            ...
            this.myDataGrid1.Size = new System.Drawing.Size(1152, 411);
            ...
            this.groupBox1.Size = new System.Drawing.Size(1158, 83);
            ...
            this.groupBox2.Size = new System.Drawing.Size(1158, 435);
            this.groupBox2.TabIndex = 5;
            this.groupBox2.TabStop = false;
            this.groupBox2.Text = "报损原因";
            // 
            // Frmbsyy
            // 
		...
		private void AddData(string ss)
		{
			...
			if (ss.Trim()=="")
			{
				ssql="select 0 序号,mc 名称,pym 拼音码,wbm 五笔码,id from yp_bsyy";
			}
			else
			{
				ssql="select 0 序号,mc 名称,pym 拼音码,wbm 五笔码,id from yp_bsyy where pym like '"+txtdm.Text.Trim()+"%' or wbm like '"+txtdm.Text.Trim()+"%' or mc like '%"+txtdm.Text.Trim()+"%' ";
			}
		...
		private void butdel_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				//看是否有过使用记录
				string ssql="select * from yk_dj where yydm="+ID+" and (ywlx='006' )";
				...
				ssql="delete from yp_bsyy where id="+ID +"";
				InstanceForm.BDatabase.DoCommand(ssql);

                //三院数据处理_____保存日志 
                if (ID > 0)
                {
                    string bz = "删除报损原因 【" + tb.Rows[nrow]["名称"].ToString() + "】 ";

                    ts.Save_log(ts_HospData_Share.czlx.yp_药品基础数据单表修改, bz, "yp_bsyy", "id", ID.ToString(), InstanceForm._menuTag.Jgbm, 0, "", out log_djid, InstanceForm.BDatabase);
                }
		...
		private void butsave_Click(object sender, System.EventArgs e)
		{
            ...
            ts_HospData_Share.ts_update_log ts = new ts_HospData_Share.ts_update_log();
            Guid log_djid = Guid.Empty;
            DataTable tb = (DataTable)this.myDataGrid1.DataSource;//将datagrid得数据源强转为DatatTable赋值
            string[] ss = new string[tb.Rows.Count];  //初始化一个以tb行数为容量的字符串数组

			try
			{
				this.butsave.Enabled=false;
                InstanceForm.BDatabase.BeginTransaction();//开始事务()
				...
				{
                    if ( tb.Rows[i].RowState == DataRowState.Unchanged )//行的状态是否改变
                        continue;
					...
					if (mc.Trim()!="")
					{
						if (ID==0)
						{
							ssql="insert into yp_bsyy(mc,pym,wbm) values('"+mc+"','"+pym+"','"+wbm+"') ";
						}
						else
						{
							ssql="update yp_bsyy set mc='"+mc+"',pym='"+pym+"',wbm='"+wbm+"' where id="+ID+" ";
						}
                        ...
                        if (ID == 0)
                        {
                            ...
                            //写一个日志
                            ts.Save_log(ts_HospData_Share.czlx.yp_药品基础数据单表修改, InstanceForm.BCurrentUser.Name + "添加报损原因 【" + mc + " 】", "yp_bsyy", "id", newid.ToString(), InstanceForm._menuTag.Jgbm, 0, "", out log_djid, InstanceForm.BDatabase);
                            ss[i] = log_djid.ToString();
                        }
                        else
                        {
                            string _ssql = "select * from yp_bsyy where id="+ID+" and mc='"+mc+"' and pym='"+pym+"'";
                            DataTable tb_x = InstanceForm.BDatabase.GetDataTable(_ssql);
                            if (tb_x.Rows.Count == 0)
                            {
                                ts.Save_log(ts_HospData_Share.czlx.yp_药品基础数据单表修改, InstanceForm.BCurrentUser.Name + "修改报损原因 【" + mc + " 】", "yp_bsyy", "id", ID.ToString(), InstanceForm._menuTag.Jgbm, 0, "", out log_djid, InstanceForm.BDatabase);
                                ss[i] = log_djid.ToString();
                            }
                            InstanceForm.BDatabase.DoCommand(ssql);//执行ssql
                        }
			...
			catch(System.Exception err)
			{
				...
				this.butsave.Enabled=true;               //保存按钮设为真
                InstanceForm.BDatabase.RollbackTransaction();//回滚事务
				MessageBox.Show("发生错误"+err.Message);
                return;
			}
