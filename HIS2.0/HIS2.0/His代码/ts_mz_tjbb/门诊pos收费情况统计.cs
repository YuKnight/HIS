...
{
    public partial class Frmsk_pos : Form
    {
        private Form _mdiParent;
        private MenuTag _menuTag;
        private string _chineseName;

        public Frmsk_pos(MenuTag menuTag, string chineseName, Form mdiParent)
        {
            InitializeComponent();
            _menuTag = menuTag;
        ...
        private void buttj_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataTable tb;
                if (_menuTag.Function_Name=="Fun_ts_mz_tjbb_jk_pos")
                    tb = TrasenFrame.Forms.FrmMdiMain.Database.GetDataTable("SP_MZSF_TJ_JK_POSTJ", parameters, 30);
                else
                    tb = TrasenFrame.Forms.FrmMdiMain.Database.GetDataTable("SP_MZSF_TJ_SK_POSTJ", parameters, 30);
                Fun.AddRowtNo(tb);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //查询条件
                string xm = "    收费员:" + cmbuser.Text;
                string swhere = label2.Text+ "从:" + dtp1.Value.ToString() + " 到:" + dtp2.Value.ToString() + xm;
                ...
                for (int j = 0; j < tb.Columns.Count; j++)
                {
                        ...
                        if (tb.Columns[j].ColumnName=="序号")
                             ws.get_Range(myExcel.Cells[5, SumColCount], myExcel.Cells[5, SumColCount]).ColumnWidth = 3;
                        else
                            ws.get_Range(myExcel.Cells[5, SumColCount], myExcel.Cells[5, SumColCount]).ColumnWidth = 7;
        ...
        private void butprint_Click(object sender, EventArgs e)
        {
            ...
            //    parameters[2].Value =label2.Text+"从 "+ dtp1.Value.ToString() + " 到 " + dtp2.Value.ToString() + "  收费员:" + cmbuser.Text.Trim();
