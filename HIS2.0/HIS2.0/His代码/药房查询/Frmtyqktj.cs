	...
	public class Frmtyqktj : System.Windows.Forms.Form
	{
		...
		private void butcx_Click(object sender, System.EventArgs e)
		{
			try
			{
                ...
                int deptid = 0;
                if (txtxm.Text.Trim() != "") 
                    hzxm = txtxm.Text.Trim();
                if (Convert.ToInt64(Convertor.IsNull(txtfph.Text.Trim(), "0")) > 0) 
                    fph = Convert.ToInt64(txtfph.Text);
                if (chkrq.Checked == true)
                {
                    date1 = dtprq1.Value.ToShortDateString() + "";
                    date2 = dtprq2.Value.ToShortDateString() + "";
                }
                if (chkyjks.Checked == true) 
                    deptid = Convert.ToInt32(cmbyjks.SelectedValue);
