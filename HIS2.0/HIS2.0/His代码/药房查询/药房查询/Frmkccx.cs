...
namespace ts_yf_cx
{
	...
	public class Frmkccx : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.myDataGrid1.Size = new System.Drawing.Size(463, 341);
            ...
            this.splitter1.Location = new System.Drawing.Point(466, 17);
            ...
            this.panel2.Location = new System.Drawing.Point(474, 17);
            this.panel2.Name = "panel2";
            this.panel2.Size = new System.Drawing.Size(391, 341);
            ...
            this.myDataGrid2.Size = new System.Drawing.Size(391, 341);
...
//					if (tbmx.Rows.Count!=0) {MessageBox.Show("对不起,这个药品的相关批号还有库存,不能禁用");return;}
		...
		private void myDataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			try
			{
				...
				string ssql="";
                if (chkykdw.Checked==false)
                    ssql="select jhj 进价,ypph 批号,ypxq 效期,kcl 库存量,dbo.fun_yp_ypdw(zxdw) 单位,cast(BDELETE as smallint)  禁用,cjid,id,(case when ykbdelete=1 then '库存禁用' else ''end) 备注 from Yf_kcph where cjid="+Convert.ToInt32(tb.Rows[nrow]["cjid"])+" and deptid="+Convert.ToInt32(cmbyjks.SelectedValue)+"";
                else
                    ssql = "select jhj 进价,ypph 批号,ypxq 效期,cast(round(kcl/dwbl,3) as float) 库存量,s_ypdw 单位,cast(a.BDELETE as smallint)  禁用,a.cjid,id,(case when ykbdelete=1 then '库存禁用' else ''end) 备注 from Yf_kcph a,yp_ypcjd b  where a.cjid=b.cjid and a.cjid=" + Convert.ToInt32(tb.Rows[nrow]["cjid"]) + " and deptid=" + Convert.ToInt32(cmbyjks.SelectedValue) + "";
		...
		private void butjy_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				string ssql="update yf_kcph set bdelete=1 where id not in( "+
					" select (select top 1 id from yf_kcph where cjid=a.cjid and deptid=a.deptid order by id desc) "+
        ...
        private void butexcel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //查询条件
                string bz = "";
                bz = bz + "药剂科室:" + cmbyjks.Text.Trim() + "    库存金额:" + tb.Compute("sum(零售金额)", "").ToString(); ;
                string swhere ="  " + bz;
                // 设置条件
