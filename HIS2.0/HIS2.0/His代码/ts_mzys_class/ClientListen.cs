...
namespace ts_mzys_class
{
    ...
    { 
        ...
        public bool CallPatient(MZHS_FZJL patient, string remoteIP,int port, out string msg)
        {
            ...
            {
                msg = "已启用分诊叫号，但是无法获取到分诊台的IP，请检查分诊诊区设置。";
                return false;
               
            }
            //add by cc 判断网络是否丢包
            Ping pingSender = new Ping();
            PingReply replyl = pingSender.Send(remoteIP, 2);
            if (replyl.Status != IPStatus.Success)
            { 
                msg = "已启用分诊叫号，但是与分诊台连接出现异常，情确认分诊台是否正常开启，谢谢！";
                return true;
            }
            //end add
