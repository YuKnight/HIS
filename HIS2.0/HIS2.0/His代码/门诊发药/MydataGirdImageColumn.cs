    ...
    public class MydataGirdImageColumn : System.Windows.Forms.DataGridTextBoxColumn
    {
       ...
       private ImageList imageList1;
        ...
        public MydataGirdImageColumn()
        {
            ...
            this.cfg8032 = new SystemCfg(8032).Config.Trim();
        ...
        protected override void Paint(Graphics g, Rectangle bounds, CurrencyManager source, int rowNum, Brush backBrush, Brush foreBrush, bool alignToRight)
        {
            ...
            if (this.DataGridTableStyle.GridColumnStyles[this.DataGridTableStyle.DataGrid.CurrentCell.ColumnNumber].GetType().Name == "MydataGirdImageColumn")
            {
                ...
                if (this.DataGridTableStyle.DataGrid[Rowindex, 1].ToString() == "-1" || this.DataGridTableStyle.DataGrid[Rowindex, 1].ToString().Trim() == "")
                {
                    index = 5;
                }
                else
                {
                    if (this.DataGridTableStyle.DataGrid[Rowindex, 1] == null)
                        this.DataGridTableStyle.DataGrid[Rowindex, 1] = 5;
                    index = Convert.ToInt32(this.DataGridTableStyle.DataGrid[Rowindex, 1].ToString());//需要用当前行
                }
            ...
            //if(Rowindex==rowNum)
            {
                flag = 1;
                int index = 5;
                if (this.DataGridTableStyle.DataGrid[rowNum, 1].ToString() == "-1" || this.DataGridTableStyle.DataGrid[rowNum, 1].ToString().Trim() == "")
                {
                    index = 5;
                }
                else
                {
                    if (this.DataGridTableStyle.DataGrid[rowNum, 1] == null)
                        this.DataGridTableStyle.DataGrid[rowNum, 1] = 5;
                     index = Convert.ToInt32(this.DataGridTableStyle.DataGrid[rowNum, 1].ToString());
                }
