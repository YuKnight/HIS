...
{
    public partial class Frmyssrtj : Form
    {
        ...
        private DataTable deptb = new DataTable();//下拉列表框绑定数据集
        public Frmyssrtj(MenuTag menuTag, string chineseName, Form mdiParent)
        {
            InitializeComponent();
            _menuTag = menuTag;
        ...
        private void cmbQt_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (rdJg.Checked == true && cmbQt.SelectedIndex != -1)
                rdJg.Checked = false;
            if (rdKj.Checked == true && cmbQt.SelectedIndex != -1)
                rdKj.Checked = false;
        }

        private void rdJg_CheckedChanged(object sender, EventArgs e)
        {
            if (cmbQt.SelectedIndex != -1 && rdJg.Checked == true)
                cmbQt.SelectedIndex = -1;
            if (cmbQt.SelectedIndex != -1 && rdKj.Checked == true)
                cmbQt.SelectedIndex = -1;
        }
    }
}